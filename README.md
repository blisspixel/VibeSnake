...
- Post-1.0 Agent Arena source developer preview where an external agent uses exact one-step play or a separate maximum-16-step event-and-lesson-target-stopping burst through a separately launched local MCP host while a human joins through the command-line watch handoff and a read-only live Godot viewer that labels actual advancement, stop facts, paused rules, and coalesced updates, with catalog-bound per-match identity independent of human cosmetics, six selectable Signal School practices, named rivals, Style Contracts, closed public intent, replay-bound lesson outcomes, verified replays, and bounded idle-session reclamation.
