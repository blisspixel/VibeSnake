...
- Post-1.0 Agent Arena source developer preview where an external agent uses exact one-step play or a separate maximum-16-step event-and-requirements-stopping burst through a separately launched local MCP host while a human joins through the command-line watch handoff and a read-only live Godot viewer that labels actual advancement, stop facts, paused rules, coalesced updates, and the match seed, state-hash prefix, plus verified replay-hash prefix a spectator can compare with the host. Catalog-bound per-match identity remains independent of human cosmetics. Eight selectable Signal School practices each expose exactly two ordered factual requirements, with replay evidence plus a bounded opposite-reversal attempt witness where required. Named rivals, five two-criterion replay-derived Style Contracts, closed public intent, verified replays, a canonical hash-linked exhibition receipt, failed-closed fresh-practice guidance, and bounded idle-session reclamation remain part of the preview.
