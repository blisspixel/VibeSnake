...
Status: Alpha 0.2.0. The source checkout is playable, all nine power-ups work through the main game loop, saves are versioned and migrated, runtime configuration is validated, and the deterministic quality suite passes. The 0.3 qualification foundation now includes a pinned Godot 4.7.1 and .NET 10.0.302 toolchain, a pure C# rules kernel, a platform-neutral persistence assembly, 177 native contract tests, 100 shared movement traces covering 25,600 steps, 35 targeted core-rule cases, 8 targeted Shield cases, normalized random-stream and respawn contracts, strict canonical state restoration, explicit `vibesnake-core@4` identity, live replay recording, bounded atomic replay storage, strict read-only import, generated continuation campaigns, retained first-divergence evidence, logical keyboard and controller actions, focus-loss safety, Shield presentation feedback, essential fallback audio, and a reproducible packaged Windows player that passes deterministic launch and artifact inspection outside the checkout. Shield is the first complete native power contract. A standalone release is not ready yet because the other eight powers, full native feature parity, macOS and Linux artifact evidence, approved content packs, and structured playtesting remain. See the [current status](docs/release/STATUS.md) and [roadmap](ROADMAP.md).

Next milestone: 0.3.0, a measured Godot and C# vertical slice with step-level parity against the Python reference and native artifact smoke tests on Windows, macOS, and Linux. The roadmap defines every dependent release through 1.0.0 without schedule estimates.
