...
try {
    ...
    foreach ($requiredLocalizationFragment in @(
        "ShellLocalization.All.Count == 723",
        "entry.Parameters.Count > 0) == 111",
    ...
    foreach ($requiredLocalizationFragment in @(
        '($localizationEvidence.stringCount -ne 723)',
        '($localizationEvidence.parameterizedStringCount -ne 111)',
