...
try {
    ...
    # Coverlet can fail on Windows runners with truncated hit-file streams or a
    # locked assembly even when every test passed. Require both product modules
    # in the report, then retry once after shutting down build servers and rebuilding.
    ...
    if (($localizationEvidence.schemaVersion -ne 1) -or
        ...
        ($localizationEvidence.stringCount -ne 509) -or
        ($localizationEvidence.parameterizedStringCount -ne 67) -or
