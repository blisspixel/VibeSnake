...
1. Read `vibesnake://agent/rules`, `vibesnake://agent/modes`, and `vibesnake://agent/identity` once per host version. Read `vibesnake://agent/styles`, `vibesnake://agent/rivals`, `vibesnake://agent/signal-school`, or `vibesnake://agent/qualification` when using those experiences.
...
Use the exact camelCase argument names, JSON types, and closed values from the discovered tool schema. `play_move` requires `action`; `play_burst` requires `initialAction` and `maximumSteps`; `start_match` takes `gameplaySeed` as a quoted decimal string. The host rejects missing, unexpected, wrong-typed, and out-of-contract argument values on every tool before game code runs, identifies the exact mismatch, lists the required and optional fields, and confirms that no match state changed. An out-of-contract value names the accepted vocabulary or range, the resource that publishes it, and the value that was received, so an unknown `lessonId`, `modeId`, `styleContractId`, `rivalPersonalityId`, `actionProfile`, `action`, `declaredIntent`, `matchHandle`, `idempotencyKey`, `gameplaySeed`, or `maximumSteps` is an ordinary argument rejection you can correct from the message. `record_passport` takes exactly one of `matchHandle` or `receiptHash`, and supplying neither or both is named the same way. Correct the argument object instead of retrying the same payload. The rejection carries no observation because it never entered match code; call `observe_match` if separate proof of the unchanged tick and state hash is wanted.
...
5. Treat each replay payload hash and final state hash as lane verification identifiers. The exhibition receipt hash-links them into `receipt_hash` for this visit and `route_identity_hash` for the line. After `archive_exhibition`, `get_exhibition_story` builds the recorded-first montage from that receipt and the named lane tapes. First-crossing style and lesson highlights are reconstructed from the agent tape; attempt-witness requirements have no tape tick. `record_passport` can fold a verified receipt into a local public record. After archiving, `get_qualification_report` classifies those receipts: voluntary `finish_match` is not a standing, completed practice is not qualification, and standings never mix divisions.
