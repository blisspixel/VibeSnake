...
During active playback, a thin two-line broadcast ticker shows the matchup, target, risk, resources, record delta, and concise decision reason without covering the action with permanent controls. The ticker briefly expands to show control hints when a broadcast starts or the viewer uses the pointer, keyboard, or controller, then returns to its thin state after three seconds of inactivity. Enter or A pauses and keeps the hints visible; resuming starts the hide timer again. Down or D-pad Down advances one step while paused, Up or D-pad Up switches the viewed rival, and Left or Right changes playback speed. H or controller L3 hides or restores the ticker without changing the run. R or controller Y restarts the same selection. After both lanes finish, the contextual controls remain visible and C or controller X starts an exact-seed human challenge under identical rules. Escape or controller B returns first to selection and then to the menu.
