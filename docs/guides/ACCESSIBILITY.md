...
| Mouse use | Nine scaled menu targets support start and major browsers. Left confirms only on those targets or steers relative to the snake head. Right performs Back except on the title menu. Middle pauses. Wheel axes navigate lists and menus, not live turns. Letterbox input is rejected. | `mouse_input.json` drives menu, settings, start, gameplay, and return through live Godot input. Physical mouse and pointer-focus review remain required. |
