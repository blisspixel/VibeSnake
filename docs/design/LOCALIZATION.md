...
- 732 unique schema-bound copy entries and 114 parameterized templates, including preview-only Agent Arena watch copy outside the supported 1.0 shell-flow set below.
...
- The whole top run HUD strip is a declared table of six cells rather than five independent fixed positions. Each cell owns a left edge, a width budget that ends before its neighbour begins, a base type size, and a floor it may not shrink past. The draw path and the readable gate read the same table, so a cell can never be rendered under rules the gate did not prove. The gate composes worst-case English for every cell of both mode presentations from the live Vibe Level catalog, the multiplier ceiling, and every reachable hunger tick of every shipped mode; it then requires no elision, a real gutter at every seam, and clearance for the scaling critical-hunger marker. At 150 percent text the smallest resolved cell is 17 point, which is still larger than the default-scale classic rules caption.
