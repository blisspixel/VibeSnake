...
Eight reviewed Python-origin cases compare normalized state and ordered power events for collection, both expiry paths, collision recovery, expiry precedence, starvation bypass, and collision recovery at the starvation deadline. Native `RepositoryChecks` reproduces their exact canonical bytes without executing rules, and the separate C# parity test remains the live consumer. Native tests separately prove deterministic spawn, a usable minimum duration, anti-stacking, saturated-board discard, restart, restoration, replay, and invalid-state rejection. See [the parity decision log](../engineering/PARITY_DECISIONS.md#pd-008-shield-collection-and-lifecycle-receive-rules-version-4).
