...
- A compact versioned Python-origin movement fixture with 100 cases and 25,600 step-level comparisons, including each command's queue-acceptance result. Native tooling reproduces its exact 999,087-byte identity while the separate C# parity test remains the live rules consumer.
...
- 1,570 native xUnit contracts pass with zero build warnings and 90 percent line and 85 percent branch floors per measured module under Coverlet 10. The current local Windows report is 94.63/85.92 percent RepositoryChecks line/branch and 94.31/86.66 percent aggregate. Independent AI-league, observed-baseline, and balance-laboratory simulations use a bounded worker set and reduce results in input order. One SnakeRun stays single-threaded. The wrapper requires RepositoryChecks, Rules, Persistence, AgentPlay, AgentViewer, and AgentHost to appear exactly once, while Coverlet's minimum-module threshold gates every instrumented module. The 0.4 acceptance target remains 90 percent branch coverage. The real Godot scene smoke uses isolated replay storage (1-4 envelopes), mirror-completes terminal death steps, browses and controls verified deterministic playback, and asserts structured log event codes including `run_dead`, `replay_finalized`, `run_start`, and `achievements_load`.
- Required Agent Arena identity, style, and Signal School evidence locks eight unique avatar, accent, and station identities, strict catalog lookup and unknown-ID rejection, Passport v4 anonymous defaults, observation profile v4, observation and result v5, viewer frame v9, rejection of legacy and mixed JSON, immutable live identity, station-name ownership shared with broadcast policy, exactly two ordered factual criteria for every Style Contract, and exactly two ordered factual requirements for every one of eight lessons. Live style and lesson progress is recomputed from rules-advanced steps; style v3 publishes threshold crossings; lesson v3 recommends `finish_match` at successful live completion; first-turn rejection evidence is separately bounded to 32 witnesses; exact retries do not add evidence; successful terminal outcomes are independently reconstructed, hash-bound, and omit retry guidance; completed lessons use lifecycle `completed`; partial lessons and ordinary nonterminal early finishes use `aborted`; incomplete and failed-closed practice exposes only fresh-session retry guidance; malformed or mismatched replay, attempt, or lifecycle evidence fails closed; and missing, unexpected, wrong-typed, or out-of-contract arguments on any of the seventeen tools report the exact mismatch before binding without changing match state. The AA-06 exhibition receipt reproduces one canonical hash for an identical exhibition, keeps `route_identity_hash` stable across a same-seed rematch under a new handle and new idempotency keys while `receipt_hash` changes, excludes presentation display time from both, breaks on any tampered fact, and fails closed for live, failed-closed, and half-verified rivalry matches. Repository tests assert the documented launchers stay executable and forward their arguments, the composed pseudo-localized overlay gate covers a dedicated verification row at 150 percent text, and a separate readable-overlay gate fails when the actual worst-case English verification or outcome row elides at maximum text scale. The frame-v9 survival block is required, ordered, and recomputed from the public observation, so a wrong exit count, wrong pressure tier, wrong held count, reordered or truncated recovery list, tampered tick value, or missing block rejects the frame. Host and plugin 0.18.0, seventeen tools, eight resources, rules resource v19 with published lifecycle, argument-binding, receipt, survival-state, archive, passport, story, and qualification vocabulary, identity resource v3, style catalog v3, Signal School resource v4, locked canonical route hashes, the deterministic capped `death-read` self-collision instruction, two non-practice evaluator seeds per lesson, 16 exact MCP-argument and structured-response measurements, observation-derived bounded burst routes that reduce calls for at least six lessons without exceeding any paired step route, and generated OKF drift are part of the same contract. The real Agent Watch smoke resolves passport presentation independently of the human selected cosmetic, verifies that human progression remains unchanged, and distinguishes live observations, verified combined lesson evidence, replay-verified style outcomes, and unavailable evidence without claiming intent or mastery.
...
- Required `localization-qualification-v1` evidence locks 734 stable English IDs, 114 exact parameterized templates, thirteen migrated shell flows, 18 resolved onboarding IDs, 24 feedback IDs, and 24 exact broadcast caption IDs. It also requires deterministic `qps-ploc` output with at least 1.3 expansion, preserved input-glyph parameters, zero missing fallback-font glyphs, logical-canvas fit at 150 percent text, seven-row Agent Arena overlay fit under composed worst-case pseudo-localized copy with grapheme-safe middle elision, no elision of the worst-case English Agent Arena survival, verification, and outcome rows at 150 percent text, a six-cell run HUD row that neither elides nor crosses a seam under composed worst-case English for both mode presentations and holds a 14-point floor at 150 percent text, and zero direct draw, prompt, static status, composed status, or audited domain-presentation expressions. It also requires every logical action named by a shell prompt to resolve through the remappable bindings document or the shell's own fixed defaults, so a prompt can never advertise a control as unbound while its key works.
...
- Required `bare-arcade-loop-qualification-v1` evidence locks one-rules-step input response, exact bounded buffer order, production-token graphical contrast, fatal-cell visibility, wrap continuity, host-smoke p95/max pacing after 30 unmeasured stabilization frames per attempt, same-step death attribution, deliberate restart, zero transient reset residue, six cross-aspect/accessibility semantic frames, linked handoff evidence, and an explicit `pending` human-feel checklist. Exported smoke retains this focused minimum-effects burst only after the exercised runtime and rendering paths complete the three-profile stabilization sequence, while preserving every sample and ceiling.
...
- Required `manual-product-matrix-handoff-v2` evidence hash-binds the V090-07 contract and locks 144 platform-flow cells, 432 complete-device flow cells, 16 mouse-capability cells, 32 platform-profile cells, four platform rows, 12 session fields, six atomic result fields, and five pending physical gates. Only passing observations earn coverage. Device or profile presence alone cannot close the matrix. The superseded `e87db6e` handoff has zero sessions, and no current physical candidate exists until content approval and a fresh Release run.
...
- Required `release-materials-handoff-v2` evidence hash-binds the strict ten-document V090-09 foundation. A structurally complete candidate record must match the expected source revision and canonical application version; declare all three artifact-manifest hashes and OS/size disclosures; cover all four physical-input evidence sets, offline and save-location claims, separate content byte counts, six structurally recognized screenshot roles, two structurally recognized video roles, and eight evidence-linked marketing claims; and hash every retained file exactly. That state is `passed: true` and `candidateMaterialComplete: true`, but `releaseAcceptance` remains `false` pending artifact-manifest reconciliation, marketing-claim approval, visible image review, and video playback review. Current pending markers prevent the alpha documents from being presented as final candidate material.
- Required schema-3 native artifact evidence records whether the Agent Arena preview is excluded. `ExportRelease` must set that state true, omit AgentPlay, AgentViewer, and AgentHost payloads plus plugin and knowledge bundles, compile out the watch route, and pass path, adversarial-name, and compiled-marker checks. Debug must record the state false. A real Windows x64 Release export passes locally; retained macOS and Linux Release evidence still belongs to the first exact three-platform candidate run.
- Native `release-rehearsal-handoff-v2` evidence hash-binds the V090-10 and material-acceptance contracts plus release-material, signing, packaging, and recovery prerequisites. A final retained record must provide candidate, previous, and manifest files for all three platforms; a separately accepted material decision; 33 passing acquisition, signature, lifecycle, optional-content, rollback, and removal cells; unchanged protected user-data hashes; the migration fixture set; complete withdrawal; and verified publish, halt, replace, and communicate roles. Strict JSON, containment, stable hashes, manifest identity, exact retained-file closure, and atomic output are automated. The staged operations and approvals are external attestations that the validator checks but does not perform.
- Native `stable-promotion-handoff-v2` evidence fixes tag and version `1.0.0`, accepts ten exact decision kinds under a separate 43-gate authority, keeps nine decisions on one unsigned reviewed cohort, requires protected signing to bridge those inputs to the byte-changing public artifacts and manifests, and hash-verifies final provenance, checksums, the approved optional pack, public install results, and the preserved release record. The exact stable compatibility acknowledgements prevent a packaging-only change from weakening scored rules, migration, category identity, missing-content visibility, accessibility regression, or offline-core promises.
...
- Required `performance-qualification-v1` evidence stabilizes each minimum, default, and maximum-safe effect profile for 30 unmeasured process frames before capturing 40 live frame samples; publishes both counts; records average, p50, p95, p99, maximum, and driver availability; and rejects shared-host gross regressions above 25 ms average or 70 ms p95. The stabilization window keeps known exported-player startup, JIT, renderer initialization, and profile transitions outside the retained distribution without hiding any measured frame. If every average passes and only a profile's p95 fails, smoke permits one profile-scoped resample while retaining already-passing rows; the affected profile must pass the unchanged envelope on its final attempt. Structural, average, or repeated tail failures do not receive another attempt. Percentile interpolation is clamped to the measured source interval, so floating-point rounding cannot report a percentile beyond the observed extrema. The p99 and maximum remain diagnostics so one scheduler outlier cannot fail an otherwise healthy shared-runner distribution, and failed packaged runs retain their measurements. The 70 ms envelope covers the observed shared macOS timer-batching mode without relaxing the 25 ms sustained-work ceiling. Shared CI cannot prove target frame-rate acceptance, but these conservative bounds can reject gross regressions. The later focused bare-loop burst separately retains its 25 ms average, 60 ms p95, and 100 ms maximum ceilings. An initial miss captures three identical 40-frame bursts and reduces one-sided scheduling delay by taking the per-frame minimum; every raw summary, the policy, and the reduced result remain in evidence. A regression present in every replicate still fails the unchanged focused envelope. The maximum scene accounts for all 2,112 board cells, 160 particles, three popups, 12 audio channels, and 2,303 of the 2,400 permitted logical draw submissions. Exact 256-step hashes must match across profiles. The 16.67 ms target is published but not accepted from an unnamed shared runner.
...
- A native deterministic public content inventory with link-free path containment, strict classification, streaming SHA-256 hashes, bounded JSON, CSV, Markdown, WAV, MPEG, and full PNG container plus decoded-scanline integrity, independent zlib and Adler-32 validation, duplicate reporting, canonical atomic output, rights state, export eligibility, and release-blocker output. Export eligibility remains zero until pack quality and credit gates pass.
- Universal 52-package CI and 4-package runtime Python locks with exact SHA-256 hashes and input digests, locked NuGet restore with transitive vulnerability audit, full-tree Ruff format and lint gates, and an executable anti-slop policy over active source and canonical docs.
...
- Hosted Windows x64, macOS Universal, and Linux x64 packaged-player gates that export and launch outside the checkout, require deterministic state hash `600f29e8919a9400`, own the player process through clean exit, reject engine warnings and leaked objects, reject Python and development payloads, detect the current checkout path and fixed development-path signatures in project payloads, reject packed export locks, and write per-file SHA-256 evidence. File counts, byte counts, and checksums remain per-build generated evidence.
...
The narrow core differential adapter mirrors the frozen Python coordinator's ordering and legacy gameplay randomness, while its focused power adapters inject positions and state and deliberately exclude presentation. Those adapters' scope is not the product kernel's capability boundary. Separate current fixtures cover normalized random-stream use and respawns, Shield, Phase Shift, Last Stand, and every remaining power; pure product tests cover the complete C# Rules kernel. Native tooling now owns exact frozen-vector rendering and freshness for 167 achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, Core Rules, and Movement vectors while their separate C# parity tests remain the live consumers. Movement uses its own tested 1,000,000-byte lifecycle rather than expanding the shared 65,536-byte small-fixture boundary. The Python side remains only a behavior and migration oracle. The remaining Python validators must receive bounded .NET replacements before retirement. Reviewed differences and excluded fields are tracked in [PARITY_DECISIONS.md](PARITY_DECISIONS.md).
...
The implemented differential foundation compares normalized state and ordered events rather than cross-language hashes because the frozen Python oracle does not implement the product PCG state or canonical JSON hash. Fixture metadata lists the exact comparison and exclusion scope. Current fixtures cover starvation ordering, collision precedence, full-grid victory, normalized random-stream use and respawns, Shield, Phase Shift, Last Stand, and the remaining powers. Any excluded field is an adapter limitation, not evidence that the authoritative C# product feature is unimplemented.
...
Dedicated performance jobs on named hardware or controlled virtual machines own product acceptance. Shared CI cannot prove target frame rate, but conservative average, tail, and hard-frame bounds reject gross regressions while preserving evidence for diagnosis.
