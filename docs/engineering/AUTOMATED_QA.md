...
- Required `performance-qualification-v1` evidence captures 40 live frame samples each for minimum, default, and maximum-safe effects; records average, p50, p95, p99, maximum, and driver availability; and rejects sustained shared-host regressions above 25 ms average or 50 ms p95. The p99 and maximum remain diagnostics so one scheduler outlier cannot fail an otherwise healthy shared-runner distribution, and failed packaged runs retain their measurements. The maximum scene accounts for all 2,112 board cells, 160 particles, three popups, 12 audio channels, and 2,303 of the 2,400 permitted logical draw submissions. Exact 256-step hashes must match across profiles. The 16.67 ms target is published but not accepted from an unnamed shared runner.
