...
- One hundred seventy-seven native xUnit contracts. Rules coverage is 91.73 percent line and 87.77 percent branch; persistence coverage is 90.73 percent line and 84.48 percent branch; aggregate coverage is 91.55 percent line, 87.26 percent branch, and 97.53 percent method. The per-module native line floor is enforced, and the real Godot scene smoke uses isolated replay storage.
...
- A deterministic 18-file clean-clone content inventory with path safety, strict classification, SHA-256 hashes, bounded JSON and decoded PNG integrity, duplicate reporting, rights state, export eligibility, and release-blocker output. Rights-unverified binaries are isolated outside the would-be public commit.
