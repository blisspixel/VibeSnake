...
The narrow core differential adapter mirrors the frozen Python coordinator's ordering and legacy gameplay randomness, while its focused power adapters inject positions and state and deliberately exclude presentation. Those adapters' scope is not the product kernel's capability boundary. Separate current fixtures cover normalized random-stream use and respawns, Shield, Phase Shift, Last Stand, and every remaining power; pure product tests cover the complete C# Rules kernel. Native tooling now owns exact frozen-vector rendering and freshness for 67 achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, and Core Rules vectors while their separate C# parity tests remain the live consumers. Movement is the next fixture migration. Its 999,087-byte corpus exceeds the shared 65,536-byte fixed-fixture limit and requires a separately designed and tested bounded large-file lifecycle. The Python side remains only a behavior and migration oracle. The remaining Python validators and fixture generators must receive bounded .NET replacements before retirement. Reviewed differences and excluded fields are tracked in [PARITY_DECISIONS.md](PARITY_DECISIONS.md).
