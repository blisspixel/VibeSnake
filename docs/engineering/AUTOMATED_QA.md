...
- Four hundred two native xUnit contracts with an 80 percent line floor per module. Recent coverlet runs measure about 90 percent line and 82 percent branch aggregate. The real Godot scene smoke uses isolated replay storage (1-4 envelopes), mirror-completes terminal death steps, and asserts structured log event codes including `run_dead`, `replay_finalized`, and `run_start`.
