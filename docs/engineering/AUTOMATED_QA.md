...
- 1,400 native xUnit contracts with 90 percent line and 85 percent branch floors per measured module under Coverlet 10. Independent AI-league, observed-baseline, and balance-laboratory simulations use a bounded worker set and reduce results in input order. One SnakeRun stays single-threaded. The current local Windows report is 92.97/86.94 percent RepositoryChecks line/branch, 95.91/88.45 percent Rules, 93.27/87.10 percent Persistence, 95.98/94.00 percent creator-validator, 94.63/86.84 percent AgentHost, 92.92/85.27 percent AgentPlay, 96.41/87.52 percent AgentViewer, and 93.97/87.08 percent aggregate. The wrapper requires RepositoryChecks, Rules, Persistence, AgentPlay, AgentViewer, and AgentHost to appear exactly once, while Coverlet's minimum-module threshold gates every instrumented module. The 0.4 acceptance target remains 90 percent branch coverage. The real Godot scene smoke uses isolated replay storage (1-4 envelopes), mirror-completes terminal death steps, browses and controls verified deterministic playback, and asserts structured log event codes including `run_dead`, `replay_finalized`, `run_start`, and `achievements_load`.
