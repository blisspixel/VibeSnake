...
- Required `performance-qualification-v1` evidence captures 40 live frame samples each for minimum, default, and maximum-safe effects; records p50, p95, p99, maximum, and driver availability; and rejects gross shared-host regressions above 50 ms p95 or 100 ms maximum. The maximum scene accounts for all 2,112 board cells, 160 particles, three popups, 12 audio channels, and 2,303 of the 2,400 permitted logical draw submissions. Exact 256-step hashes must match across profiles. The 16.67 ms target is published but not accepted from an unnamed shared runner.
