...
The narrow core differential adapter mirrors the frozen Python coordinator's ordering and legacy gameplay randomness, while its focused power adapters inject positions and state and deliberately exclude presentation. Those adapters' scope is not the product kernel's capability boundary. Separate current fixtures cover normalized random-stream use and respawns, Shield, Phase Shift, Last Stand, and every remaining power; pure product tests cover the complete C# Rules kernel. Native tooling now owns exact frozen-vector rendering and freshness for achievement candidates, Last Stand, Phase Shift, Shield, and Remaining Powers while their separate C# parity tests remain the live consumers. Core Rules is the next bounded fixture migration because its 35 reviewed cases form the smallest remaining independent corpus and its 57,031 bytes fit the shared fixed-fixture bound. The Python side remains only a behavior and migration oracle. The remaining Python validators and fixture generators must receive bounded .NET replacements before retirement. Reviewed differences and excluded fields are tracked in [PARITY_DECISIONS.md](PARITY_DECISIONS.md).
