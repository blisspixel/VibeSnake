...
- 1,045 native xUnit contracts with 90 percent line and 85 percent branch floors per measured module under Coverlet 10. The current local Windows report is 95.85/88.53 percent Rules line/branch, 93.54/87.17 percent Persistence line/branch, 95.98/94.00 percent creator-validator line/branch, 94.68/87.83 percent AgentHost line/branch, 93.78/88.45 percent AgentPlay line/branch, 95.18/86.58 percent AgentViewer line/branch, and 94.40/87.68 percent aggregate. The wrapper requires Rules, Persistence, AgentPlay, AgentViewer, and AgentHost to appear exactly once, while Coverlet's minimum-module threshold gates every instrumented module. The 0.4 acceptance target remains 90 percent branch coverage. The real Godot scene smoke uses isolated replay storage (1-4 envelopes), mirror-completes terminal death steps, browses and controls verified deterministic playback, and asserts structured log event codes including `run_dead`, `replay_finalized`, `run_start`, and `achievements_load`.
- Required AA-03 identity and style evidence locks eight unique avatar, accent, and station identities, strict catalog lookup and unknown-ID rejection, Passport v3 anonymous defaults, observation profile v3, observation and result v4, viewer frame v6, rejection of legacy and mixed JSON, immutable live identity, station-name ownership shared with broadcast policy, and exactly two ordered factual criteria for every Style Contract. Live style progress is recomputed from rules-advanced steps, successful terminal style outcomes are independently reconstructed from and hash-bound to the verified replay, and malformed or mismatched replay evidence fails closed. Host and plugin 0.6.0, eight tools, seven resources, and generated OKF drift are part of the same contract. The real Agent Watch smoke resolves passport presentation independently of the human selected cosmetic, verifies that human progression remains unchanged, and distinguishes live observations from replay-verified style outcomes without claiming intent or mastery.
