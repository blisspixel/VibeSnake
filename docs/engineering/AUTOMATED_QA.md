...
- 1,493 native xUnit contracts with 90 percent line and 85 percent branch floors per measured module under Coverlet 10. Independent AI-league, observed-baseline, and balance-laboratory simulations use a bounded worker set and reduce results in input order. One SnakeRun stays single-threaded. The current local Windows report is 93.58/85.70 percent RepositoryChecks line/branch, 95.91/88.45 percent Rules, 93.27/87.10 percent Persistence, 95.98/94.00 percent creator-validator, 95.09/87.43 percent AgentHost, 92.92/85.27 percent AgentPlay, 96.41/87.52 percent AgentViewer, and 93.97/86.63 percent aggregate. The wrapper requires RepositoryChecks, Rules, Persistence, AgentPlay, AgentViewer, and AgentHost to appear exactly once, while Coverlet's minimum-module threshold gates every instrumented module. The 0.4 acceptance target remains 90 percent branch coverage. The real Godot scene smoke uses isolated replay storage (1-4 envelopes), mirror-completes terminal death steps, browses and controls verified deterministic playback, and asserts structured log event codes including `run_dead`, `replay_finalized`, `run_start`, and `achievements_load`.
...
The narrow core differential adapter mirrors the frozen Python coordinator's ordering and legacy gameplay randomness, while its focused Shield adapter injects positions and state and deliberately excludes presentation. That adapter's scope is not the product kernel's capability boundary. Separate current fixtures cover normalized random-stream use and respawns, Shield, Phase Shift, Last Stand, and every remaining power; pure product tests cover the complete C# Rules kernel. The Python side remains only a behavior and migration oracle. Stable promotion, the other remaining validators, and fixture generators must receive bounded .NET replacements before retirement. Reviewed differences and excluded fields are tracked in [PARITY_DECISIONS.md](PARITY_DECISIONS.md).
