...
The first six bounded validator slices are complete. Native `RepositoryChecks` owns documentation discovery and links, changelog contract-release uniqueness, canonical product-version parsing and package mapping, cross-file alignment, active authored-file discovery, credentials and signing material, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, Python placeholders, candidate-freeze policy, frozen-surface resolution, active-baseline verification, deterministic baseline preparation, both Python dependency-lock profiles, the approved 1024 by 1024 project-logo PNG identity, and source and packaged Agent Plugin validation. Its 139 focused contracts cover malformed inputs, excluded and generated trees, unsafe patterns, duplicate JSON fields, policy and activation drift, deterministic hashes and results, ordered path-plus-byte input identities, exact pins and SHA-256 hashes, resolver version and warning policy, timeouts, atomic replacement, tampering, PNG structure and dimensions, canonical plugin metadata, minimal-skill frontmatter, bounded tree traversal, links, stdio containment, exact package components, streaming checksums, and independent command routes. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly uses its version route after locked restore. Three native runners no longer install the Agent Plugin validator's Python dependencies; their pinned Python setup remains for the separate Agent Host package validator. The superseded Python entry points and policy modules for these checks are gone. Ruff remains the complete Python syntax parser, and the shared Python version helper remains temporarily because later release-assembly scripts still import it.

The next bounded candidate is deterministic station-badge generation and freshness checking. Content, screenshot, release, and fixture-generation migrations follow as separate reviewed contracts.
