...
The first ten bounded validator slices are complete. Native `RepositoryChecks` owns documentation discovery and links, changelog contract-release uniqueness, canonical product-version parsing and package mapping, cross-file alignment, active authored-file discovery, credentials and signing material, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, Python placeholders, candidate-freeze policy, frozen-surface resolution, active-baseline verification, deterministic baseline preparation, both Python dependency-lock profiles, the approved 1024 by 1024 project-logo PNG identity, source and packaged Agent Plugin validation, deterministic station-badge generation and exact-byte freshness, deterministic content-inventory generation, integrity, freshness, and release readiness, staged README screenshot capture plus freshness, and V090-09 release-material structural qualification. Its 187 focused contracts cover malformed inputs, excluded and generated trees, unsafe patterns, duplicate JSON fields, policy and activation drift, deterministic hashes and results, ordered path-plus-byte input identities, exact pins and SHA-256 hashes, resolver version and warning policy, timeouts, atomic replacement, tampering, complete PNG integrity and dimensions, canonical plugin metadata, minimal-skill frontmatter, bounded tree traversal, links, stdio containment, exact package components, streaming checksums, closed station definitions, integer-only visual rendering, canonical PNG structure, snapshot identities, metadata-only drift, bounded badge paths, strict inventory schemas and classification, media corruption, zlib and Adler-32 integrity, screenshot source fingerprints, isolated Godot capture, release-material contract and document identity, closed candidate metadata, declared platform sizes, structural media formats, stable retained snapshots and hashes, tree and byte limits, explicit final-acceptance blockers, and independent command routes. Release-material output is canonical LF `release-materials-handoff-v2` written by atomic replacement. Structural completion can pass with `candidateMaterialComplete: true`, but `releaseAcceptance` remains false pending artifact reconciliation, claim approval, visible image review, and video playback. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly uses its version and inventory-release routes after locked restore. Three native runners no longer install the Agent Plugin validator's Python dependencies; their pinned Python setup remains for the separate Agent Host package validator. The superseded Python entry points and duplicate Python CI routes for these checks are gone. Ruff remains the complete Python syntax parser; the shared Python version helper remains because later release-assembly scripts still import it, and the frozen inventory parity library remains because Python pack tools still import it.

The next bounded candidate is release rehearsal because it must validate and consume a later accepted release-material decision rather than the raw structural handoff. Stable promotion follows because it consumes both materials and rehearsal. The Release matrix remains an earlier candidate producer, while fixture-generation migrations remain independent reviewed contracts.
