...
| --- | --- | --- |
| `check_agent_interop_upstream.py` | Scheduled read-only integrity probe for three reviewed HTTPS resources after native local baseline qualification | Retain as isolated network scaffolding until a native remote-integrity gate has explicit timeout, response-size, redirect, and failure contracts. It is not a local policy authority. |
| `validate_agent_host_package.py` | Closed AA-10 package manifest, inventory, provenance, checksums, and isolated-user-data policy | Next selected slice now that package validation can consume one native interop contract identity. |
...
The Release matrix remains an earlier candidate producer. The selected next slice is `validate_agent_host_package.py` because the local interop identity it consumes is now native and its closed package manifest, inventory, provenance, checksum, and isolation contracts can move without performing packaging or publication.
