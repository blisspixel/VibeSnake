...
The remaining Python CI and package-tool graph was audited on 2026-09-01. Its boundaries and ordered disposition are:

| Remaining surface | Dependency boundary | Retirement disposition |
| --- | --- | --- |
| `check_agent_interop.py`, `compute_agent_contract_digests.py`, and the interop portion of `close_agent_preview.py` | Standard-library local baseline and digest validation, plus an opt-in read-only upstream HTTP drift probe | Next bounded slice: move the deterministic local baseline and digest authority into `RepositoryChecks`. Keep the scheduled network probe separate until a native remote-integrity gate has its own timeout and failure contracts. |
| `validate_agent_host_package.py` | Closed AA-10 package manifest, inventory, provenance, checksums, and isolated-user-data policy | Migrate after the local interop authority so package validation consumes one native contract identity. |
| `check_manual_product_matrix.py` and `check_external_validation.py` | Deterministic retained-evidence handoff validation with no product runtime dependency | Migrate as paired evidence validators after Agent Host package validation. They do not perform the human or external work they qualify. |
| `check_release_matrix.py` and `assemble_unsigned_preview.py` | Three-platform package, manifest, provenance, and unsigned-preview assembly | Migrate together after the retained-evidence validators so one native release graph owns both input validation and assembly. |
| `content_packs.py` and `assemble_radio_pack.py` | Imports the frozen `vibesnake.content` manifest and inventory library | First move the shared content qualification model into native tooling, then replace both callers without duplicating pack semantics. |
| `scripts/manual/*` | Human review preparation, audio analysis, playback, and external FFmpeg execution | Retain until equivalent native operator workflows exist. These tools gather or prepare human evidence and are not safe leaf-validator deletions. |
...
| `product_version.py`, `_checkout.py`, Python packaging metadata, locks, wheel/sdist assembly, and the source-reference release path | Helpers or distribution edges shared by remaining Python callers | Remove with their last dependents in the final audited cleanup, then rerun source, artifact, documentation, license, and dependency inventories. |

The Release matrix remains an earlier candidate producer. The selected next slice is the deterministic local portion of `check_agent_interop.py` because it is the smallest independent authority, directly guards the newly native knowledge inputs, and can move without mixing scheduled network behavior into ordinary repository qualification.
