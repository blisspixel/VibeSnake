...
Status: Native source default; frozen-oracle procedures retained (2026-08-30).
...
| Content inventory / packs | Native `RepositoryChecks` inventory + shared pack allowlists | Partial | Native schema-1 generation, freshness, integrity, and release readiness are authoritative; exportEligible=0 |
...
The first thirteen bounded validator slices are complete. Native `RepositoryChecks` owns documentation discovery and links, changelog contract-release uniqueness, canonical product-version parsing and package mapping, cross-file alignment, active authored-file discovery, credentials and signing material, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, Python placeholders, candidate-freeze policy, frozen-surface resolution, active-baseline verification, deterministic baseline preparation, both Python dependency-lock profiles, the approved 1024 by 1024 project-logo PNG identity, source and packaged Agent Plugin validation, deterministic station-badge generation and exact-byte freshness, deterministic content-inventory generation, integrity, freshness, and release readiness, staged README screenshot capture plus freshness, V090-09 release-material structural qualification, V090-10 release-rehearsal qualification, stable-promotion qualification, and exact achievement-candidate fixture rendering/freshness. Its focused contracts cover malformed inputs, excluded and generated trees, unsafe patterns, duplicate JSON fields, policy and activation drift, deterministic hashes and results, ordered path-plus-byte input identities, exact pins and SHA-256 hashes, resolver version and warning policy, timeouts, atomic replacement, tampering, complete PNG integrity and dimensions, canonical plugin metadata, minimal-skill frontmatter, bounded tree traversal, links, stdio containment, exact package components, streaming checksums, closed station definitions, integer-only visual rendering, canonical PNG structure, snapshot identities, metadata-only drift, bounded badge paths, strict inventory schemas and classification, media corruption, zlib and Adler-32 integrity, screenshot source fingerprints, isolated Godot capture, release-material contract and document identity, closed candidate metadata, declared platform sizes, structural media formats, accepted-material, rehearsal, and unsigned-to-signed stable-promotion cross-binding, stable retained snapshots and hashes, tree and byte limits, explicit final-acceptance blockers, and independent command routes. Release-material, rehearsal, and stable-promotion outputs are canonical LF JSON written by atomic replacement. Structural materials can complete while final acceptance remains false. Rehearsal can accept only an exact retained staged record with a separate accepted-material decision and same-revision artifact manifests. Stable promotion accepts only ten authorized decision kinds, one unsigned reviewed cohort, a protected signing bridge to final public bytes, exact installs and preservation, and complete retained closure. Achievement-candidate tooling reproduces the four reviewed Python-origin vectors as exact 2,682-byte canonical LF JSON without executing native rules; the live C# parity consumer remains independent. Its fixed path, bounded reads and sibling enumeration, link and portable-alias rejection, durable same-directory replacement, cleanup, and exact self-verification form the next fixture-migration pattern. The validators never perform external operations or approvals. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly uses its version and inventory-release routes after locked restore. Three native runners no longer install the Agent Plugin validator's Python dependencies; their pinned Python setup remains for the separate Agent Host package validator. The superseded Python entry points and duplicate Python CI routes for these checks are gone. The focused native repository-check surface now has 240 contracts. Ruff remains the complete Python syntax parser; the shared Python version helper remains because later release-assembly scripts still import it, and the frozen inventory parity library remains because Python pack tools still import it.

The next bounded candidate is native Last Stand fixture generation and freshness. Achievement-candidate migration now provides the fixed-path, canonical LF, exact-byte, bounded-read, link-safe, and atomic-replacement pattern without making native parity self-referential. Last Stand is the next-smallest targeted corpus at five cases with an existing native consumer; Phase Shift follows before the larger or RNG-coupled corpora. The Release matrix remains an earlier candidate producer, while fixture-generation migrations remain independent reviewed contracts.
