...
The first four bounded validator slices are complete. Native `RepositoryChecks` owns documentation discovery and links, changelog contract-release uniqueness, canonical product-version parsing and package mapping, cross-file alignment, active authored-file discovery, credentials and signing material, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, Python placeholders, candidate-freeze policy, frozen-surface resolution, active-baseline verification, deterministic baseline preparation, and both Python dependency-lock profiles. Its 79 focused contracts cover malformed inputs, excluded and generated trees, unsafe patterns, duplicate JSON fields, policy and activation drift, deterministic hashes and results, ordered path-plus-byte input identities, exact pins and SHA-256 hashes, resolver version and warning policy, timeouts, atomic replacement, tampering, and independent command routes. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly uses its version route after locked restore. The superseded Python entry points and policy modules for these checks are gone. Ruff remains the complete Python syntax parser, and the shared Python version helper remains temporarily because later release-assembly scripts still import it.

The next bounded slice is native project-logo validation. It is the smallest remaining authoritative Python validator, has a closed PNG signature, dimension, and SHA-256 contract, and can remove one CI and pre-commit Python invocation without touching player or content bytes. Agent Plugin, generated badge, content, screenshot, release, and fixture-generation migrations follow as separate reviewed contracts.
