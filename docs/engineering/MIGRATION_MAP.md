...
The first nine bounded validator slices are complete. Native `RepositoryChecks` owns documentation discovery and links, changelog contract-release uniqueness, canonical product-version parsing and package mapping, cross-file alignment, active authored-file discovery, credentials and signing material, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, Python placeholders, candidate-freeze policy, frozen-surface resolution, active-baseline verification, deterministic baseline preparation, both Python dependency-lock profiles, the approved 1024 by 1024 project-logo PNG identity, source and packaged Agent Plugin validation, deterministic station-badge generation and exact-byte freshness, deterministic content-inventory generation, integrity, freshness, and release readiness, and staged README screenshot capture plus freshness. Its 170 focused contracts cover malformed inputs, excluded and generated trees, unsafe patterns, duplicate JSON fields, policy and activation drift, deterministic hashes and results, ordered path-plus-byte input identities, exact pins and SHA-256 hashes, resolver version and warning policy, timeouts, atomic replacement, tampering, complete PNG integrity and dimensions, canonical plugin metadata, minimal-skill frontmatter, bounded tree traversal, links, stdio containment, exact package components, streaming checksums, closed station definitions, integer-only visual rendering, canonical PNG structure, snapshot identities, metadata-only drift, bounded badge paths, strict inventory schemas and classification, media corruption, zlib and Adler-32 integrity, screenshot source fingerprints, isolated Godot capture, tree and byte limits, release blockers, and independent command routes. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly uses its version and inventory-release routes after locked restore. Three native runners no longer install the Agent Plugin validator's Python dependencies; their pinned Python setup remains for the separate Agent Host package validator. The superseded Python entry points and duplicate Python CI routes for these checks are gone. Ruff remains the complete Python syntax parser; the shared Python version helper remains because later release-assembly scripts still import it, and the frozen inventory parity library remains because Python pack tools still import it.

The next bounded candidate is the release-material foundation validator. It is the earliest remaining release-qualification dependency, its strict document and candidate-evidence contract is already defined, and moving it first avoids rebuilding later rehearsal and stable-promotion validation around a temporary Python authority. The dependent release and fixture-generation migrations follow as separate reviewed contracts.
