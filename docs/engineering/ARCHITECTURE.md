...
`VibeSnake.AgentPlay` depends only on Rules and is the sole external-match state owner. It projects a versioned closed public logical-state observation, serializes requests per session, rejects stale or illegal actions without stepping, mirrors each accepted action into a verified replay, and optionally advances an independent same-seed built-in rival. `VibeSnake.AgentHost` adapts that contract to local MCP stdio and uses Persistence only for explicit application-owned replay save. `VibeSnake.AgentViewer` reads full public frames from a one-time same-user local-pipe capability and has no action surface. Godot can render those frames, but viewer state, timing, loss, and input cannot affect match rules or replay identity. Agent paths construct no human progression, achievement, score-history, preference, or household-comparison store. See [AGENT_PLAY.md](AGENT_PLAY.md) for the protocol and operational contract.

The preview is also a compile-time release boundary. Normal development and Debug builds define `AGENT_ARENA_PREVIEW`; Godot `ExportRelease` removes the AgentPlay and AgentViewer project references and compiles out the watch route. Schema-3 artifact inspection rejects preview assemblies, host and plugin material, integration bundles, and compiled command-line markers. A local Windows Release export passes this gate; the exact three-platform candidate evidence remains required.

### Cores and threads

Modern machines have many cores. Vibe Snake still keeps **one rules run on one thread**. That is the product, not a leftover: scored steps, event order, replay hashes, and fair scores are sequential contracts. Splitting one SnakeRun across cores would race the body, RNG streams, and hashes.

Extra cores are already used where work is independent:
...
Native `RepositoryChecks` scans source assets without loading them into either game runtime, requires every file to match exactly one human-reviewed policy rule, bounds traversal and bytes, validates the complete supported media containers, and generates deterministic hashes, integrity results, duplicate links, rights state, release blockers, and export eligibility. It writes canonical LF JSON through an atomic sibling replacement and requires exact checked-in freshness on all three CI platforms. Pure C# pack parsing and resolution validate dependency-free core and station-specific optional manifests against approved inventory, compatibility, credits, and exact file metadata. Godot uses that native service and isolates optional failure from core play. The native player continues to ship no legacy source assets while export eligibility remains zero.
...
The first 21 bounded retirement slices are complete. Seven closed native fixture renderers reproduce 167 reviewed Python-origin vectors across achievement candidates, Last Stand, Phase Shift, Shield, Remaining Powers, Core Rules, and Movement as exact canonical LF files of 2,682, 3,596, 3,534, 4,489, 9,548, 57,031, and 999,087 bytes. Each renderer remains separate from its live native rules consumer. Six small fixtures share the 65,536-byte lifecycle; Movement has a separate 1,000,000-byte lifecycle and review boundary. The twentieth slice adds a separate source-derived Agent Knowledge renderer that applies the fixed-path lifecycle independently to five closed Open Knowledge Format 0.2 concepts. The twenty-first moves the bounded offline agent-interoperability baseline, source alignment, documentation pins, and versioned contract-digest authority into the same native tool while leaving the scheduled HTTP probe isolated.
