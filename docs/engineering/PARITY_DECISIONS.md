...
Shared fixtures declare `vibesnake-core@4` and state their exact injected-or-normalized randomness policy. The remaining corpora are regenerated and compared byte for byte in Python CI before C# consumes them. The four achievement-candidate vectors retain their reviewed `python-core-reference-v3` origin, while native tooling reproduces their exact 2,682 bytes without executing C# rules. A semantic fixture change requires the relevant parity entry and a reviewed rules decision.
