...
| `tests/fixtures/shared/remaining_powers_rules_v1.json` | Reviewed Python-origin production Slow-Mo, Boost, Magnet, Bait, Gluttony, and Segment Detach corpus, canonically rendered and freshness-checked by native `RepositoryChecks` | C# xUnit parity test | 9 targeted cases covering collection, activation, countdown, magnet pull, Gluttony no-growth scoring, bait marking, Segment Detach obstacles, composed tempo, normalized state, and ordered power events |
...
Shared fixtures declare `vibesnake-core@4` and state their exact injected-or-normalized randomness policy. The movement corpus remains regenerated and compared byte for byte in Python CI before C# consumes it. The four achievement-candidate, five Last Stand, six Phase Shift, eight Shield, nine Remaining Powers, and 35 Core Rules vectors retain their reviewed Python origins; native tooling reproduces their exact 2,682, 3,596, 3,534, 4,489, 9,548, and 57,031 bytes without executing C# rules. Their separate parity tests remain the live behavior consumers. Movement is the next migration, but its 999,087-byte identity requires a separate bounded large-file lifecycle. A semantic fixture change requires the relevant parity entry and a reviewed rules decision.
