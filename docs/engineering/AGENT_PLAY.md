...
An Agent Passport contains only a caller-declared bounded ID, policy version, display name, color, shed, station affinity, a fixed symbolic observation profile, and either the step or burst action profile. The current host does not establish global identity or long-term stability. `agent-default`, `open-frequency`, arbitrary valid `#RRGGBB`, shed, and station values are preview presentation placeholders, not closed-catalog identities. The passport is public presentation data for the current exhibition, not semantic memory. Vibe Snake never stores prompts, reasoning, credentials, provider responses, or agent-authored executable code.
