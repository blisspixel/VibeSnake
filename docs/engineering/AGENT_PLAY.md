...
7. Wrong `play_move` and `play_burst` field names still fail during SDK argument binding before application code can return a typed observation. Tool descriptions and the skill name the exact fields and tell clients to reread the discovered schema after this generic pre-invocation error. A structured host rejection remains impossible until the SDK exposes binding failures to the tool boundary.
