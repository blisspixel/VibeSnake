...
This gate verifies the checksum-bound editor and official export template, rejects export warnings, launches outside the checkout, requires a deterministic smoke hash, inspects required platform payloads, rejects Python runtimes, environment files, and development content, scans project payloads for source-machine paths, and writes a schema 3 per-file manifest containing the archive and executable checksums plus the Release-only Agent Arena exclusion state. Release-mode CI also requests 100 consecutive clean launch probes from the read-only install, each with a distinct fresh external profile and a 30-second timeout. The aggregate matrix requires all 300 platform launches before provenance.
...
- 615 deterministic tests pass locally on Python 3.14 at 87.15 percent coverage; hosted CI runs the same suite across Python 3.11, 3.12, 3.13, and 3.14. This suite is temporary migration scaffolding under the [Python retirement plan](MIGRATION_MAP.md#repository-wide-python-retirement), not a product runtime. Optional-content absence is an explicit tested state rather than a skipped release assumption.
...
- 1,002 native C# contract tests pass on .NET 10 with 90 percent line and 85 percent branch floors per measured module under Coverlet 10. The current local Windows report measures Rules at 95.80/88.44 percent line/branch, Persistence at 93.54/87.17 percent, creator validation at 95.98/94.00 percent, AgentHost at 94.57/87.83 percent, AgentPlay at 94.37/89.32 percent, AgentViewer at 94.46/87.99 percent, and aggregate native coverage at 94.42/87.79 percent. The gate rejects a coverage report that omits Rules, Persistence, AgentPlay, AgentViewer, or AgentHost, while Coverlet's minimum-module threshold gates every instrumented module; `scripts/test_native.ps1` refreshes the exact report after each run. The 0.4 acceptance target remains 90 percent branch coverage.
- Agent identity tests require exactly eight unique catalog avatars, accents, and stations; valid anonymous defaults; ordinal and case-sensitive lookup; rejection of unknown IDs; strict Passport v2 JSON; rejection of Passport v1 and mixed legacy fields; immutable viewer identity; observation v3, result v3, and frame v5 boundaries; host 0.5.0; rules resource v5; identity resource v1; eight tools; and seven resources. Godot smoke separately proves that Agent Watch resolves its passport avatar, accent, and station while leaving the human selected cosmetic unchanged.
...
- Required `player-data-recovery-qualification-v1` evidence covers five separate reset categories, exact target confirmation, cancel-without-write, backup-before-removal, copy/hash integrity, corrupt restore rejection, no-overwrite conflicts, successful restore, visible backup location, raw keyboard/controller routes, and a faulted background operation that cancels pending quit instead of hiding the failure.
...
- Hosted Windows x64, macOS Universal, and Linux x64 Debug qualification players launch outside the checkout and report the same state hash `600f29e8919a9400`.
- Every qualified Release distribution must pass isolated replay storage, complete SHA-256 inventory, required Rules, Persistence, and Game payload, project-payload path, no-Python, no-export-lock, no-checkout-path, no-engine-warning, and no-leaked-object checks. File counts, byte counts, and checksums remain per-build generated evidence rather than unversioned documentation claims.
- The Debug payload passes the checksum-bound schema 3 inspector and reproducible qualification packaging. Release inspection additionally requires the preview-exclusion bit, rejects Agent Arena payload paths, and scans compiled project payloads for preview markers. Exact per-build hashes and byte counts remain in generated evidence rather than becoming stale documentation. Signing readiness correctly marks Debug artifacts as non-promotable.
...
The CI workflow declares Python 3.11, 3.12, 3.13, and 3.14. The complete suite passes locally on Python 3.14 and in hosted CI on all four supported versions. The latest pushed `main` revision has confirmed green hosted CI; every new revision still requires its own hosted confirmation after push.
...
- Performance smoke retains `performance-qualification-v1` for minimum, default, and maximum-safe profiles with 30 unmeasured stabilization frames followed by 40 retained frames each. Both counts are published and checked. It rejects missing statistics, incorrect percentile ordering, gross shared-host regressions, incomplete full-board stress geometry, over-budget particles/audio/logical draw submissions, full-screen flashes, or rules hash drift. A tail-only profile with every average under budget permits one profile-scoped resample; passing sibling rows remain retained, and the affected profile must pass the unchanged ceilings on its final attempt. The focused minimum-effects bare-loop burst then runs in the same process with its own 30-frame stabilization and 40 retained frames, after those exercised runtime and rendering paths have completed the three-profile stabilization sequence. The aggregate matrix requires 360 accepted profile samples, exact profile shapes and budgets, and one rules hash across Windows, macOS, and Linux. Dummy-backend driver draw-call unavailability is recorded explicitly. The 60 FPS and 16.67 ms target is accepted only from named minimum hardware, not ordinary CI.
...
- Localization smoke retains `localization-qualification-v1` evidence for 604 stable IDs, 87 exact named-parameter templates, thirteen migrated shell flows, 18 onboarding IDs, 24 feedback IDs, and 24 broadcast caption IDs. It also requires deterministic `qps-ploc` expansion, unchanged input-glyph parameters, zero missing fallback-font glyphs, maximum-text-scale logical-canvas fit, composed five-row Agent Arena overlay fit at 150 percent text with grapheme-safe middle elision, and zero direct draw, prompt, static status, composed status, or audited domain-presentation expressions.
...
[.github/workflows/ci.yml](../../.github/workflows/ci.yml) runs the Python quality matrix, retains seeded QA evidence, builds and tests the pure C# rules on Windows, macOS, and Linux, and runs the real Godot headlessly on all three systems. Each native rules runner assembles and checksum-validates the framework-dependent Agent Plugin, expands its declared package host path, and completes the stateless official C# SDK transcript through that packaged assembly. Each native player runner also installs a checksum-verified platform template, exports its packaged player, launches it outside the checkout, inspects and hashes the artifact, and uploads its manifest. Full qualified player bundles are uploaded for tagged and manually dispatched runs. The native jobs use locked NuGet dependencies, formatting checks, warnings as errors, 90 percent C# line and 85 percent branch floors for every instrumented module, exact required-module checks for Rules, Persistence, AgentPlay, AgentViewer, and AgentHost, and checksum-verified engine archives.

Hosted execution is active on the canonical public repository: native player export and smoke jobs run on Windows, macOS, and Linux. The next automation work is bounded .NET replacement of the remaining Python validators and fixture generators plus retained Release execution of the implemented Agent Arena exclusion, signing, and promotion gates. Physical-device, retained scaling screenshot, and visible-presentation claims still require human evidence beyond headless hosted execution.
