...
- 533 deterministic tests pass locally on Python 3.14 with the public radio pack present; hosted CI runs the same suite across Python 3.11, 3.12, 3.13, and 3.14, while radio integration cases may skip when the optional pack is absent.
- Python line coverage is 87.12 percent on Python 3.14, above the enforced 80 percent gate that CI applies to every supported interpreter.
- 892 native C# contract tests pass on .NET 10 with 90 percent line and branch floors per measured module. Rules measures 95.77/90.50 percent line/branch, Persistence measures 94.14/90.40 percent, creator validation measures 95.98/94.00 percent, and aggregate native coverage is 94.79/90.47 percent. The gate rejects a coverage report that omits Rules or Persistence, and Coverlet's minimum-module threshold also gates the creator validator; `scripts/test_native.ps1` refreshes the exact report after each run.
