...
- 586 deterministic tests plus 14 subtests pass locally on Python 3.14 at 87.15 percent coverage; hosted CI runs the same suite across Python 3.11, 3.12, 3.13, and 3.14. This suite is temporary migration scaffolding under the [Python retirement plan](MIGRATION_MAP.md#repository-wide-python-retirement), not a product runtime. Optional-content absence is an explicit tested state rather than a skipped release assumption.
...
- 964 native C# contract tests pass on .NET 10 with 90 percent line and 85 percent branch floors per measured module under Coverlet 10. Rules measures 95.77/88.43 percent line/branch, Persistence measures 93.55/87.12 percent, creator validation measures 95.98/94.00 percent, AgentHost measures 95.50/88.23 percent, AgentPlay measures 93.70/87.37 percent, AgentViewer measures 95.69/95.55 percent, and aggregate native coverage is 94.42/87.77 percent. The gate rejects a coverage report that omits Rules, Persistence, AgentPlay, AgentViewer, or AgentHost, while Coverlet's minimum-module threshold gates every instrumented module; `scripts/test_native.ps1` refreshes the exact report after each run. The 0.4 acceptance target remains 90 percent branch coverage.
