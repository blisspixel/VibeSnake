...
- Station badge checks render all eight icons with the pinned Pillow graph and its embedded reduced Aileron Regular subset, verify that exact font identity, then compare exact PNG bytes with the checked-in set.
