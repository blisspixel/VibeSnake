...
- 902 native C# contract tests pass on .NET 10 with 90 percent line and 85 percent branch floors per measured module under Coverlet 10. Rules measures 95.77/88.69 percent line/branch, Persistence measures 93.54/87.12 percent, creator validation measures 95.98/94.00 percent, and aggregate native coverage is 94.41/87.76 percent. The gate rejects a coverage report that omits Rules or Persistence, and Coverlet's minimum-module threshold also gates the creator validator; `scripts/test_native.ps1` refreshes the exact report after each run. The 0.4 acceptance target remains 90 percent branch coverage.
...
The CI workflow declares Python 3.11, 3.12, 3.13, and 3.14. The complete suite passes locally on all four versions, and the latest pushed `main` revision has confirmed green hosted CI. Every new revision still requires its own hosted confirmation after push.
