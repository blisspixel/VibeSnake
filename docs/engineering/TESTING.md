...
- 985 native C# contract tests pass on .NET 10 with 90 percent line and 85 percent branch floors per measured module under Coverlet 10. The latest completed three-platform hosted baseline measures Rules at 95.77/88.43 percent line/branch, Persistence at 93.53-93.55/87.12 percent, creator validation at 95.98/94.00 percent, AgentHost at 94.76/85.13-87.16 percent, AgentPlay at 96.40/88.21 percent, AgentViewer at 95.69/95.55 percent, and aggregate native coverage at 94.56-94.58/87.73-87.78 percent. Ranges reflect platform-specific generated branches. The gate rejects a coverage report that omits Rules, Persistence, AgentPlay, AgentViewer, or AgentHost, while Coverlet's minimum-module threshold gates every instrumented module; `scripts/test_native.ps1` refreshes the exact report after each run. The 0.4 acceptance target remains 90 percent branch coverage.
...
- The Debug payload passes the checksum-bound schema 2 inspector and reproducible qualification packaging. Exact per-build hashes and byte counts remain in generated evidence rather than becoming stale documentation. Signing readiness correctly marks Debug artifacts as non-promotable.
...
Hosted execution is active on the canonical public repository: native player export and smoke jobs run on Windows, macOS, and Linux. The next automation work is bounded .NET replacement of the remaining Python validators and fixture generators, a dedicated 1.0 artifact assertion that excludes every Agent Arena preview assembly and entry point, and retained Release execution through the protected signing and promotion operations. Physical-device, retained scaling screenshot, and visible-presentation claims still require human evidence beyond headless hosted execution.
