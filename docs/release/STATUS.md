...
The gated 1.0 target remains Godot 4 .NET with deterministic pure C# rules and first-class Windows, macOS, and Linux players. The native foundation builds, tests, exports, and smokes on hosted Windows, macOS, and Linux runners. Shield, Phase Shift, and Last Stand are complete native power contracts. The game is not release-ready: the other six powers, full native feature parity, pack export approval (inventory export-eligible count is still zero), and structured player validation remain.
...
| Native contract tests | 193 passing on .NET 10 with an 80 percent line floor per module |
...
| Power-ups | Python complete; native Shield, Phase Shift, and Last Stand complete | All nine powers change real Python runs. Collision-recovery powers are pure C# with shared parity fixtures. The other six powers remain unported. |
...
| Automated gameplay QA | Foundation working | Seeded policies, invariants, property-generated input, replay hashes, JSON reports. Native parity retains first-divergence bundles. Full powers, DDA, AI, and presentation campaigns still depend on the completed deterministic engine. |
...
| Target technology | Qualification in progress | Godot 4.7.1 and .NET 10.0.302 pinned. Pure C# kernel covers core rules plus Shield, Phase Shift, and Last Stand. Hosted multi-platform player smoke exists. Full feature parity with the Python reference does not. |
