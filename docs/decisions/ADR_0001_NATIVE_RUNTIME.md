...
- The rules contract exposes `vibesnake-core@4`, canonical state uses schema 3 and `fnv1a64-canonical-json-v4` (session achievement counters restore with the run), and schema 1 replay envelopes retain the canonical initial state, ordered logical actions, checkpoints, observed outcome, compatibility diagnostics, and SHA-256 payload integrity.
