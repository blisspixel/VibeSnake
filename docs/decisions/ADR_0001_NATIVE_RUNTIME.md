...
- The native solution builds with warnings as errors and passes 1,539 tests with zero build warnings. Under Coverlet 10, the current local Windows aggregate is 94.28 percent line and 86.73 percent branch coverage, with RepositoryChecks at 94.56 percent line and 86.06 percent branch. Current 90 percent line and 85 percent branch floors apply per measured module, with a retained 90 percent branch target for 0.4 acceptance.
- Shared Python-to-C# fixtures cover 100 movement traces with 25,600 steps, core rules, all nine powers, and the achievement-candidate product path. Achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, and Core Rules corpora retain 67 reviewed Python-origin vectors under native exact-byte freshness ownership, while separate live C# consumers prove behavior. The six closed renderers preserve exact 2,682-byte, 3,596-byte, 3,534-byte, 4,489-byte, 9,548-byte, and 57,031-byte canonical LF identities. Reviewed parity decisions own intentional differences.
