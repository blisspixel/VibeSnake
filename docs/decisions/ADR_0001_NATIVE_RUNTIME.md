...
- The native solution builds with warnings as errors and passes 1,502 tests. Under Coverlet 10, the current local Windows aggregate is 93.99 percent line and 86.65 percent branch coverage, with RepositoryChecks at 93.70 percent line and 85.78 percent branch. Current 90 percent line and 85 percent branch floors apply per measured module, with a retained 90 percent branch target for 0.4 acceptance.
- Shared Python-to-C# fixtures cover 100 movement traces with 25,600 steps, core rules, all nine powers, and the achievement-candidate product path. The achievement corpus retains reviewed Python-origin vectors under native exact-byte freshness ownership, while a separate live C# consumer proves behavior. Reviewed parity decisions own intentional differences.
