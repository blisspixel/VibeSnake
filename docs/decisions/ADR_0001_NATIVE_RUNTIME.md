...
- The native solution builds with warnings as errors and passes 1,511 tests. Under Coverlet 10, the current local Windows aggregate is 94.06 percent line and 86.62 percent branch coverage, with RepositoryChecks at 93.89 percent line and 85.69 percent branch. Current 90 percent line and 85 percent branch floors apply per measured module, with a retained 90 percent branch target for 0.4 acceptance.
- Shared Python-to-C# fixtures cover 100 movement traces with 25,600 steps, core rules, all nine powers, and the achievement-candidate product path. Achievement-candidate and Last Stand corpora retain reviewed Python-origin vectors under native exact-byte freshness ownership, while separate live C# consumers prove behavior. Reviewed parity decisions own intentional differences.
