...
| Pure rules | `VibeSnake.Rules` has no Godot reference and implements PCG32, bounded input, movement, food, growth, combo scoring, starvation, collision precedence, win state, the complete Shield lifecycle and recovery contract, immutable typed events, explicit restart, snapshots, strict canonical JSON schema 2 restoration, explicit `vibesnake-core@4` identity, a canonical replay envelope, live mirror-verified recording, and `fnv1a64-canonical-json-v3` hashes | Add near misses, the other eight powers, AI inputs, and the remaining reviewed semantics |
...
| Content boundary | Schema 1 policy and generated inventory account for all 18 clean-clone assets and 95,377 bytes with exact hashes, bounded JSON and decoded PNG integrity, duplicates, roles, pack intent, rights status, and zero current export approvals; 423 files and 740,801,845 bytes of unresolved, rejected, generated, analytical, copied-research, superseded-document, private-path, retired-tooling, historical-production, and working material are isolated in the ignored `archive/source-assets` collection | Bind any reconsidered service-assisted media to provider-aware provenance, build one audited admission tool with a pinned media toolchain, add full media analysis, lyrics review, and credits, define core and optional pack schemas, and allow exports to read only approved manifests |
