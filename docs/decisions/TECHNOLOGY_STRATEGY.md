...
| Player artifacts | The current Windows x64 Release bundle launches outside the checkout, emits hash `600f29e8919a9400`, resolves the packaged content inventory, writes and verifies isolated replays, and passes a complete 199-file inventory with no Python runtime, `.env` variant, checkout path, export lock, engine warning, leaked object, or unapproved radio asset. The schema 2 inspector requires and path-scans the Rules, Persistence, and Game payloads, binds the editor executable to the executable inside the pinned archive, and the output gate proves deterministic qualification packaging. Exact byte and checksum identities are retained in each build's artifact manifest instead of copied into this strategy document. | Retain current macOS Universal and Linux x64 artifacts, then expand physical input, audio, display, and user-data checks on all three |
