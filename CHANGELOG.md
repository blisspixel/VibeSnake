...
- Added a local stdio MCP 2026-07-28 host with six schema-bound tools and six resources, bounded opaque sessions, explicit path-free replay save, protocol transcript coverage, and no network listener. A same-user one-time named-pipe capability feeds a read-only Godot watch screen without giving the viewer match control.
...
- Synchronized README, testing, architecture, quality, packaging, and release-status documentation with the current 601-test Python baseline, 964-test native coverage report, MCP 2026-07-28 and official C# SDK 2.2.0 pin, Agent Plugins 1.0.0 Working Draft boundary, Open Knowledge Format 0.2 boundary, and the exact source-versus-assembled contents of `player-latest`.
