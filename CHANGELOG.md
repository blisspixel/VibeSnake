...
- Added a local stdio MCP 2026-07-28 host, now exposing seven schema-bound tools and six resources, bounded opaque sessions, explicit path-free replay save, protocol transcript coverage, and no network listener. A same-user one-time named-pipe capability feeds a read-only Godot watch screen without giving the viewer match control.
