...
- Completed the Agent Arena viewer's closed rejection copy for wrong-profile and exhausted-mutation-budget responses, corrected disconnect status so it never implies that an unfinished or failed-closed match already has a verified replay, and advanced the Agent Plugin guidance-only contract to `0.2.1`.
...
- Bumped the Agent Host and Agent Plugin preview contracts to 0.2.0 for the seventh MCP tool and action-profile change. Corrected MCP 2026-07-28 documentation to its stateless, no-initialize model and added a legacy-client rejection transcript. One machine-readable interoperability baseline now owns reviewed versions, schema digests, and review dates; ordinary CI validates it offline and a read-only weekly workflow reports upstream Agent Plugins schema drift.
...
- Added the versioned `vibesnake-agent-viewer-frame-v2` contract with explicit action acceptance, rejection reason, exact end reason, failed-closed state, and verified-result availability. The Godot watch screen now shows the passport color, shed, and station, while viewer disconnects no longer imply that a replay already exists.
...
- Synchronized README, testing, architecture, quality, packaging, and release-status documentation with the current 612-test Python baseline, 985-test native coverage report, MCP 2026-07-28 and official C# SDK 2.2.0 pin, the Agent Plugins 1.0.0 status discrepancy between its Published normative repository and Working Draft website, Open Knowledge Format 0.2 boundary, and the exact source-versus-assembled contents of `player-latest`.
