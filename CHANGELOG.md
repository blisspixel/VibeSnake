...
- Unified bare-loop and performance qualification around the same 60 ms shared-host p95 ceiling, retaining the 25 ms sustained-average ceiling, the 100 ms bare-loop hard-frame cap, and named-hardware 60 FPS acceptance. A profile with a tail-only shared-runner spike may be measured once more without discarding already-passing sibling profiles; every profile still must pass the unchanged ceilings, and structural, sustained, or repeated failures remain fatal. Percentile interpolation is clamped to its source samples so floating-point rounding cannot report p99 above the observed maximum.
