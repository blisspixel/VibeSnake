...
- Fixed the isolated `player-latest` publisher to pass its repository explicitly to every GitHub CLI release operation. The write-token job remains checkout-free and can now update the qualified release without relying on ambient Git state.
