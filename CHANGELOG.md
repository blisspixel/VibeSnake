...
- Unified bare-loop and performance qualification around the same 60 ms shared-host p95 ceiling, retaining the 25 ms sustained-average ceiling, the 100 ms bare-loop hard-frame cap, and named-hardware 60 FPS acceptance. Every bare-loop attempt and every stress profile now stabilizes for 30 unmeasured process frames before measurement so exported-player startup, JIT, renderer initialization, profile transitions, and shared-runner scheduling do not contaminate the retained distribution. Exported smoke retains its bare-loop burst after the complete three-profile qualification sequence, so the exercised runtime and rendering paths complete their stabilization sequence before the focused minimum-effects sample begins. The performance evidence publishes and validates its 30-frame stabilization and 40-frame measurement counts. A profile with a tail-only shared-runner spike may be measured once more without discarding already-passing sibling profiles; every profile still must pass the unchanged ceilings, and structural, sustained, or repeated failures remain fatal. Percentile interpolation is clamped to its source samples so floating-point rounding cannot report p99 above the observed maximum.
