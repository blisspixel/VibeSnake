...
- Established the earlier `vibesnake-agent-viewer-frame-v2` contract with explicit action acceptance, rejection reason, exact end reason, failed-closed state, and verified-result availability. The Godot watch screen added passport color, shed, and station, while viewer disconnects no longer implied that a replay already existed. The 0.3.0 work above supersedes this wire identity with v3.
