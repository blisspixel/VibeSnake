...
- Hardened the Agent Arena truth boundary so the viewer rejects missing, duplicate, unknown, or noncanonical JSON fields; impossible episode and mode facts; catalog-drifting lesson, identity, or rival facts; contradictory style arithmetic; and mismatched terminal style outcomes before presentation. The watch overlay now distinguishes live lesson progress, target hit awaiting finalization, replay-verified target met or not met, and failed-closed replay evidence unavailability. Host input parsing applies the same required-member, duplicate-member, closed-enum, and unknown-member discipline. The authoritative native qualification passes 1,045 tests with 94.40 percent aggregate line coverage and 87.68 percent aggregate branch coverage; every measured module remains above the 90 percent line and 85 percent branch floors.
