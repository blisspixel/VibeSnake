...
| --- | --- | --- | --- |
| 1 | **Close the 0.3 product-evidence gaps** - run physical controller and hot-plug routes, audio-device changes, real window-manager/display checks, retained platform captures, declared-hardware frame measurement, and visible feel/recovery review against the retained `e87db6e` packages | Automation already covers the contracts; these are now the highest-risk claims that automation cannot honestly close | Yes - first native alpha and later feel claims |
| 2 | **Review and approve the first export packs (V030-08/09)** - listen to the technically passing 12-track `the_bureau` review set, approve or reject exact source replacements, then produce non-zero `exportEligible` core/radio allowlists with credits, full decode, loudness, true peak, silence, listening, and exact manifests | All 95 source MP3 files decode but fail the complete policy as-is; one lossless station review set now clears the automated screen without changing source or approval state, making listening the next honest decision | Yes - first native alpha |
