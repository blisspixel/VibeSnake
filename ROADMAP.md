...
| 5 | **Retire Python scaffolding in bounded slices (V030-13)** - move project-logo validation next, then replace Agent Plugin, generated visual, content, screenshot, release, and fixture-generation paths with tested .NET equivalents while external reviews or credentials are pending | The logo check is the smallest closed remaining validator, protects an approved release asset, and removes one Python CI and pre-commit route without changing player bytes | No - safe parallel technical work only |
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **1,306** native xUnit contracts pass with 90 percent line and 85 percent branch floors per measured module under Coverlet 10; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; native documentation, version, source-policy, candidate-freeze, and dependency-lock qualification, rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
| V030-13 Python retirement | In progress | The shipped source player and every native artifact already use Godot plus .NET without Python. Native `RepositoryChecks` owns documentation, product-version, source-policy, candidate-freeze, and dependency-lock qualification with 79 focused contracts and exact `uv 0.11.33` generation. Python remains frozen test-only scaffolding for remaining validators, fixture generation, and the oracle; no product features may land there. [MIGRATION_MAP.md](docs/engineering/MIGRATION_MAP.md#repository-wide-python-retirement) defines the ordered exit gates. | Move remaining validators starting with project-logo, then fixture generation, to .NET; replace the Python CI matrix with equivalent native cross-platform gates; then remove the Python player, tests, packaging metadata, dependency locks, and source-snapshot path in one clean audited sequence. |
...
- **Progress (not closed):** the native `RepositoryChecks` command now owns documentation discovery and links, unique changelog contract releases, canonical SemVer parsing, PEP 440 mapping, version alignment across `VERSION`, `pyproject.toml`, the native application identity, and the frozen Python fallback, plus active authored-file discovery, credential and signing-material exclusion, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, Python placeholders, candidate-freeze policy, frozen-surface resolution, active-baseline verification, atomic baseline preparation, both Python dependency-lock profiles, and exact `uv 0.11.33` generation with ordered input identity and atomic replacement. Seventy-nine focused contracts cover malformed and missing inputs, excluded and generated trees, duplicate declarations and JSON fields, encoded and invalid paths, fenced examples, inline and multiline placeholders, unsafe globs, prerequisite and activation drift, deterministic hashes and diagnostics, resolver versions, warnings, timeouts, tampering, and independent command routes. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly runs its version route after locked restore. The superseded Python command files, source-policy module, candidate-freeze command, dependency-lock command, and their replaced tests are removed. Ruff retains full Python syntax ownership. Project-logo validation is next. Content, screenshot, remaining release validators, fixture generation, and the Python runtime still remain.
