...
**First seventeen safe parallel retirement slices complete:** native `RepositoryChecks` replaces the Python documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, station-badge, content-inventory, README screenshot, release-material, release-rehearsal, stable-promotion, achievement-candidate fixture, Last Stand fixture, Phase Shift fixture, Shield fixture, and Remaining Powers fixture commands. Its 270 focused malformed-input and deterministic-result contracts run through the combined route on all three hosted operating systems. The four achievement-candidate, five Last Stand, six Phase Shift, eight Shield, and nine Remaining Powers vectors retain reviewed Python provenance and exact 2,682-byte, 3,596-byte, 3,534-byte, 4,489-byte, and 9,548-byte canonical identities without executing native rules. Their separate C# parity tests remain the live behavior consumers. A shared fixed-fixture layer bounds generated and checked bytes, verifies stable reads, rejects symbolic/reparse-point paths and portable case aliases, revalidates fixed paths immediately before flushed same-directory replacement, preserves primary failures during cleanup, and self-verifies exact output. Release-material, rehearsal, and stable-promotion qualification remain non-mutating evidence gates with external approval and execution explicit. Core Rules is next because its 35 reviewed cases form the smallest remaining independent corpus and its 57,031 bytes still fit the bounded fixture lifecycle.
...
| 6 | **Continue bounded Python retirement and preview isolation in parallel** - migrate Core Rules fixture generation next, then the larger movement fixture path with a tested .NET equivalent while preserving supported-artifact exclusion on every candidate | Native achievement-candidate, Last Stand, Phase Shift, Shield, and Remaining Powers tooling shares a hardened fixed-fixture lifecycle without making any parity consumer self-referential; the 35-case, 57,031-byte Core Rules corpus is the smallest remaining independent fixture set and still fits that bounded lifecycle | No - safe parallel technical work only |
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **1,532** native xUnit contracts pass with zero build warnings and 90 percent line and 85 percent branch floors per measured module under Coverlet 10; RepositoryChecks measures 94.35/85.92 percent line/branch and aggregate native coverage measures 94.20/86.69 percent; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; native documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, station-badge, content-inventory, README screenshot, release-material structural, release-rehearsal retained-record, stable-promotion retained-record, exact achievement-candidate, Last Stand, Phase Shift, Shield, and Remaining Powers fixture rendering/freshness, and Agent Plugin qualification, rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
| --- | --- | --- | --- |
| V030-01 Python reference | Frozen oracle; retirement in progress | Seeded policies, schema 2 reports, per-step invariants, property-generated commands, JSON action traces, immediate replay, 100 movement fixtures with 25,600 steps, 35 targeted core-rule cases, power fixtures (Shield/Phase Shift/Last Stand/remaining), explicit queue acceptance, stable off-path food, normalized random-stream use and respawns, ordered events, explicit `vibesnake-core@4` and randomness-policy declarations, strict source and pack-content gates, and 616 passing Python tests plus 14 subtests at 87.27 percent line coverage measured locally while hosted CI covers 3.11 through 3.14 | Keep the oracle green only until its validators and fixture generators have native replacements; **do not add product features or optional refactors in Python**. Follow V030-13 for complete removal. |
