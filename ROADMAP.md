...
**First three safe parallel retirement slices complete:** native `RepositoryChecks` replaces the Python documentation, product-version, source-policy, and candidate-freeze commands, has 62 focused malformed-input and deterministic-result contracts, runs on all three hosted operating systems, and owns tagged-alpha version validation after locked restore. Source policy covers active authored-file discovery, credentials and signing material, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, and Python placeholders without starting Python. Candidate-freeze policy now resolves the 93-file surface, validates inactive or frozen state, and prepares deterministic atomic SHA-256 baselines. Ruff retains complete Python syntax ownership while the frozen oracle exists. The next bounded slice is dependency-lock validation and generation because every remaining Python validator depends on the two reproducible graphs, direct `uv` orchestration has a closed deterministic contract, and the wrapper, module, and six tests can then be removed without changing player behavior.
...
| 5 | **Retire Python scaffolding in bounded slices (V030-13)** - move dependency-lock validation and direct `uv` generation next, then replace later validators and fixture generation with tested .NET equivalents while external reviews or credentials are pending | The lock contract protects every remaining Python tool and can be removed as one deterministic slice without diverting product work or duplicating frozen-oracle features | No - safe parallel technical work only |
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **1,289** native xUnit contracts pass with 90 percent line and 85 percent branch floors per measured module under Coverlet 10; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; native documentation, version, source-policy, and candidate-freeze qualification, rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
