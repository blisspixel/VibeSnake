...
**First 18 safe parallel retirement slices complete:** native `RepositoryChecks` replaces the Python documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, station-badge, content-inventory, README screenshot, release-material, release-rehearsal, stable-promotion, achievement-candidate fixture, Last Stand fixture, Phase Shift fixture, Shield fixture, Remaining Powers fixture, and Core Rules fixture commands. Its focused malformed-input and deterministic-result contracts run through the combined route on all three hosted operating systems. The four achievement-candidate, five Last Stand, six Phase Shift, eight Shield, nine Remaining Powers, and 35 Core Rules vectors retain reviewed Python provenance. The six closed native renderers preserve all 67 vectors and their exact 2,682-byte, 3,596-byte, 3,534-byte, 4,489-byte, 9,548-byte, and 57,031-byte canonical LF identities without executing native rules. Their separate C# parity tests remain the live behavior consumers. A shared fixed-fixture layer bounds generated and checked bytes through an inclusive 65,536-byte limit, verifies stable reads, rejects symbolic/reparse-point paths and portable case aliases, revalidates fixed paths immediately before flushed same-directory replacement, preserves primary failures during cleanup, and self-verifies exact output. Release-material, rehearsal, and stable-promotion qualification remain non-mutating evidence gates with external approval and execution explicit. Movement is next, but its 999,087-byte corpus requires a separate bounded large-file lifecycle and design review.

**Focused repository-check surface:** 277 contracts after the Core Rules migration.
...
| 6 | **Continue bounded Python retirement and preview isolation in parallel** - design and migrate the movement fixture path with a tested bounded large-file .NET lifecycle while preserving supported-artifact exclusion on every candidate | Six native fixture renderers now share a hardened 65,536-byte fixed-fixture lifecycle without making any parity consumer self-referential; the next 999,087-byte corpus crosses that small-file boundary and needs separate capacity, read, write, replacement, and failure contracts | No - safe parallel technical work only |
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **1,539** native xUnit contracts pass with zero build warnings and 90 percent line and 85 percent branch floors per measured module under Coverlet 10; RepositoryChecks measures 94.56/86.06 percent line/branch and aggregate native coverage measures 94.28/86.73 percent; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; native documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, station-badge, content-inventory, README screenshot, release-material structural, release-rehearsal retained-record, stable-promotion retained-record, exact achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, and Core Rules fixture rendering/freshness, and Agent Plugin qualification, rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
| --- | --- | --- | --- |
| V030-01 Python reference | Frozen oracle; retirement in progress | Seeded policies, schema 2 reports, per-step invariants, property-generated commands, JSON action traces, immediate replay, 100 movement fixtures with 25,600 steps, 35 targeted core-rule cases, power fixtures (Shield/Phase Shift/Last Stand/remaining), explicit queue acceptance, stable off-path food, normalized random-stream use and respawns, ordered events, explicit `vibesnake-core@4` and randomness-policy declarations, strict source and pack-content gates, and 612 passing Python tests plus 14 subtests at 86.95 percent line coverage measured locally while hosted CI covers 3.11 through 3.14 | Keep the oracle green only until its validators and fixture generators have native replacements; **do not add product features or optional refactors in Python**. Follow V030-13 for complete removal. |
...
| V030-13 Python retirement | In progress | The shipped source player and every native artifact already use Godot plus .NET without Python. Native `RepositoryChecks` owns documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, deterministic station-badge, deterministic content-inventory, staged README screenshot capture/freshness, release-material structural qualification, release-rehearsal qualification, stable-promotion qualification, and exact achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, and Core Rules fixture rendering/freshness with a shared hardened fixed-path lifecycle. Python remains frozen test-only scaffolding for remaining validators, fixture generation, pack-tool parity, and the oracle; no product features may land there. [MIGRATION_MAP.md](docs/engineering/MIGRATION_MAP.md#repository-wide-python-retirement) defines the ordered exit gates. | Design the bounded large-file lifecycle for the 999,087-byte movement fixture and migrate it without expanding the 65,536-byte small-fixture boundary; replace the Python CI matrix with equivalent native cross-platform gates; then remove the Python player, tests, packaging metadata, dependency locks, and source-snapshot path in one clean audited sequence. |
...
7. While human sessions, content decisions, or credentials are pending, continue V030-13 after the completed documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, station-badge, content-inventory, screenshot, release-material, release-rehearsal, stable-promotion, achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, and Core Rules slices. Design and migrate the 999,087-byte movement fixture generator under its own bounded large-file lifecycle. Finish or defer each slice before candidate freeze. Remove Python only after the complete replacement matrix passes; never treat new Python features as roadmap progress.
