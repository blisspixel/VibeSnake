...
| --- | --- | --- | --- |
| 1 | **Godot shell depth** - interactive remap capture, glyphs, physical multi-controller evidence, scaling matrix, authored/fallback audio stress (bindings browse + achievements browse already land) | Players receive the Godot artifact; shell debt is the visible product | Yes - human/feel gates and 0.3 slice close |
| 2 | **Native packaging (V030-10)** - wire pure release manifest validator into packaging, store installers, signing separation (schema 2 + declared shapes landed) | Continuous export smokes exist; store-shaped install still does not | Yes - install promise and artifact gates |
...
| 4 | **C# rules polish (0.4 foundations)** - corpus compaction, shell transition completeness, replay browse/playback UI, not new Python features | Rules kernel is largely ported; remaining work is evidence depth and Godot UX | Partial - 0.4 gate, not 0.3 feature port |
...
**Open human gates (do not claim 0.8 while any remain):** pack `exportEligible` > 0 with approved manifests; declared-hardware frame evidence; physical controller play pass; installer/archive release shapes; recovery/feel observation.
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **451** native xUnit contracts pass with coverage floors; shared movement, core-rule, power, and achievement-candidate product-path fixtures pass; rules throughput, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 2, input bindings, structured logging, diagnostics, multi-power synergy campaigns, and achievements browse report gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
| --- | --- | --- | --- |
| V030-01 Python reference | Freeze in progress (oracle only) | Seeded policies, schema 2 reports, per-step invariants, property-generated commands, JSON action traces, immediate replay, 100 movement fixtures with 25,600 steps, 35 targeted core-rule cases, power fixtures (Shield/Phase/Last Stand/remaining), explicit queue acceptance, stable off-path food, normalized random-stream use and respawns, ordered events, explicit `vibesnake-core@4` and randomness-policy declarations, strict source and pack-content gates, and 466 passing Python tests plus 3 expected optional-radio skips on Python 3.11 through 3.14 | Keep oracle green and fixture generators deterministic; **do not add product features in Python**. Remaining oracle work is optional corpus compaction and defect-driven corrections only |
...
| V030-07 decision close | In progress | **451** native contracts pass with coverage floors; pure rules throughput evidence JSON with config hash and conservative CI floor; presentation_frames smoke evidence; content inventory parse and export-eligibility lock; architecture purity tests (filesystem, process, sleep/delay, console, wall-clock, env bans); multi-stream RNG bank wired into Create; personality validation; preferences schema 2 store; input bindings schema 1 store with InputMap apply, restore-defaults, and pure TryRemapAction conflict checks; shell accessibility hotkeys with persistence and presentation gates; pure controller connection tracker with disconnect pause safety; content pack budgets, ContentBudgetReport, ContentTimingReport, ContentEligibilityReport, and full budget predicates; offline diagnostics reports with optional config hash; virtual viewport transform applied in shell draw; near-miss and combo-expired default-on after dual-runtime regen; achievement candidates product-gated with dedicated product-path fixture; schema 3 session counters; multi-power synergy campaigns; achievements browse report; effective `RunConfig` config hash and `RunScoreIdentity` fair-score categories; formatting, editor smoke, logical input and lifecycle smoke, finite fallback-audio and full-portfolio power feedback smoke, cadence-drain contracts, remaining-power parity fixtures, delta-reduced divergence bundles, replay recording, isolated atomic storage, strict import, hosted multi-platform player smoke, and artifact inspection exist | Presentation p50/p95/p99 on declared hardware, physical input, audio-device stress, complexity comparison, and final gate review |
...
| V030-09 content boundaries | In progress | Schema 1 defines one dependency-free `vibesnake.core`, station-specific radio packs, exact inventory allowlists, canonical encoding, semantic-version and `vibesnake-core@4` ranges, rights-derived credits, file hashes, station track lists, actionable compatibility codes, build-time qualification, and optional failure isolation; public radio tracks are already assigned to pack id `vibesnake-radio`; pure C# `ContentInventory` parses the published inventory; `ContentPackBudgets` declare core/radio ceilings; `ContentBudgetReport` measures inventory totals against ceilings (including working-set); `ContentTimingReport` compares measured scan/cold-start timings to declared ceilings; media-type prefix counts; Godot `ContentService` resolve codes (Ready/NotFound/NotExportEligible/InvalidPath), media-type listing/counts, and smoke deny non-exportEligible packaging; artifact inspection fails closed on blocked inventory paths and non-zero eligibility until allowlists land | Full media loading/decoding service, first approved core and station manifests, core-only vertical-slice proof, exact allowlist matching once eligibility is non-zero, removal and tamper UI, and installed-artifact budget measurements |
| V030-10 artifact and directory contracts | In progress | Outside-checkout output, platform-specific required Rules, Persistence, and Game payload checks, per-file SHA-256 manifests, prohibited-content rules, deterministic source-path mapping, macOS ZIP inspection, isolated explicit replay roots, spaces and non-ASCII replay paths, bounded file-count and byte budgets, cross-process transaction locking, no-overwrite atomic replay writes, published [USER_DATA.md](docs/engineering/USER_DATA.md) platform roots and layouts, and pure C# `ReleaseArtifactManifest` schema 2 parse/validate matching `inspect_native_artifact.ps1` (platform payload patterns, byte-sum integrity, declared installer/archive shapes: portable-folder / app-bundle-zip) | Wire pure validator into CI inspection consumers, read-only install path drills, optional-pack removal, signing separation, and store-channel packaging |
| V030-11 evidence pipeline | In progress | Hosted CI runs Python QA, native rules, Godot/player smoke, and inspection on Windows, macOS, and Linux; continuous `player-latest` release packaging refreshes from `main`; content-eligibility-evidence-v1 JSON from inventory gate | Broader differential reports, screenshots, content-pack checks in artifacts, dependency inventories, and provenance retention for release candidates |
...
- **Progress (not closed):** Schema 1 envelope with rules identity, RNG and state-hash algorithms, config hash, optional shell `appVersion`, initial state, step commands, checkpoints, outcome, and SHA-256 integrity; live mirror recorder; bounded atomic store; verification and compatibility codes. Remaining: UI browser/playback, timestamps in envelope if not only filenames, and seed fields as explicit top-level metadata beyond embedded initial state.
