...
**Historical Release evidence complete:** [Release run 32421705560](https://github.com/blisspixel/VibeSnake/actions/runs/32421705560) retained the exact `e87db6e` Windows x64, macOS Universal, and Linux x64 packages, manifests, matrix, and provenance. The Release matrix passed 300 clean launches, 600,000 deterministic reliability comparisons, 300 spectator restarts, 21 injected faults, 360 performance samples, and 24 maximum-text-scale display rows. Later product changes supersede those bytes as a physical-review candidate. A fresh content-bearing Release matrix is required before physical acceptance.

**Human-review contracts prepared:** `review_radio_copies.py` rehashes every prepared FLAC and creates a 12-track listening template covering full playback, clipping/distortion, edges, relative level, station fit, and sustained comfort on both headphones and speakers. That hash-bound listening handoff remains current at 0 of 12 decisions. The historical `e87db6e` physical handoff is structurally qualified but superseded and contains zero sessions. Schema-2 physical observations now bind every passing flow to one device, any mouse capability, active settings profiles, platform, candidate revision, and artifact hash. Acceptance requires 144 platform-flow cells, 432 complete-device flow cells, 16 mouse-capability cells, and 32 platform-profile cells. A new physical handoff will be generated only after content approval and a fresh Release run.

**First thirteen safe parallel retirement slices complete:** native `RepositoryChecks` replaces the Python documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, station-badge, content-inventory, README screenshot, release-material, release-rehearsal, stable-promotion, and achievement-candidate fixture commands, has 240 focused malformed-input and deterministic-result contracts, runs the combined route on all three hosted operating systems, and owns tagged-alpha version and content-release validation after locked restore. Source policy covers active authored-file discovery, credentials and signing material, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, and Python placeholders without starting Python. Candidate-freeze policy resolves the 93-file surface, validates inactive or frozen state, and prepares deterministic atomic SHA-256 baselines. Dependency qualification validates both reproducible graphs and directly orchestrates exact `uv 0.11.33` generation with ordered input identity, exact pins and hashes, fatal warnings and timeouts, and atomic replacement. Project-logo qualification pins the approved 1024 by 1024 PNG signature, IHDR dimensions, and SHA-256 identity without an image library. Agent Plugin qualification adds required canonical metadata, bounded strict input and tree traversal, link and launch containment, exact packaged components, and streaming checksum verification. Station-badge qualification owns eight closed station definitions, project glyphs, integer-only drawing, a canonical RGB PNG encoder, bounded link-safe paths, atomic replacement, and exact-byte freshness that rejects valid metadata-only drift. Content-inventory qualification reproduces the 114-file schema-1 snapshot exactly, bounds all inputs and traversal, rejects links and policy ambiguity, streams hashes and media validation, independently verifies PNG zlib and Adler-32 integrity, reports duplicate relationships and release blockers, and writes canonical LF JSON atomically. Screenshot qualification stages all four Godot captures outside the evidence directory, validates their complete PNG structure and closed canonical manifest before replacement, writes the manifest last, and checks native presentation-source freshness on every supported CI host without claiming cross-host pixel identity. Release-material structural qualification validates the closed ten-document foundation, strict bounded candidate records, exact revision and version identity, declared platform/input/size/save-location fields, structural media formats, stable retained snapshots and hashes, and evidence-linked claims before atomically writing canonical LF `release-materials-handoff-v2` evidence. A structurally complete candidate remains unaccepted until artifact-manifest reconciliation, marketing-claim approval, visible image review, and video playback review close separately. Native release-rehearsal qualification binds a separate machine-readable material-acceptance authority, strict retained evidence, exact candidate and manifest identities, 33 platform-operation cells, withdrawal, protected-data preservation, and four operational roles into canonical `release-rehearsal-handoff-v2` evidence carrying the revision required downstream. It validates external attestations but performs no platform operation or approval. Native stable-promotion qualification now binds the protected 1.0 contract to a separate ten-kind acceptance authority, rejects generic favorable JSON, preserves the unsigned reviewed cohort, requires platform signing to bridge those inputs to byte-changing signed public artifacts and provenance, and emits canonical `stable-promotion-handoff-v2` evidence. It validates retained external attestations but performs no approval, signing, tagging, upload, installation, or publication. Native achievement-candidate qualification preserves four reviewed Python-origin vectors, reproduces their exact 2,682 canonical LF bytes without executing C# rules, rejects stale bytes, links, and portable aliases, and replaces only the fixed fixture atomically. The existing parity test remains the independent live C# consumer. Last Stand fixture generation is the next bounded candidate because its five targeted cases already have a native consumer and are the next-smallest reviewed corpus.

| Priority | Next work | Why this order | Blocks the next promotion? |
| --- | --- | --- | --- |
| 1 | **Execute and apply the first content decisions (V030-08/09)** - fill the hash-bound 12-track `the_bureau` headphone/speaker record, approve or reject exact source replacements, then produce non-zero `exportEligible` core/radio allowlists with credits, full decode, loudness, true peak, silence, listening, and exact manifests | All 95 source MP3 files decode but fail the complete policy as-is; content, inventory, and manifest changes must land before candidate identity is frozen | Yes - first native alpha |
| 2 | **Cut a fresh exact 0.3 Release candidate** - freeze the content-bearing revision, run the complete three-platform Release matrix, retain its packages and provenance outside ephemeral CI storage, and generate a new candidate-bound physical workspace | Current `main` includes player-visible changes after historical run `32421705560`; physical evidence must bind the revision intended for publication | Yes - physical acceptance |
| 3 | **Execute the corrected physical product review** - run all platform-flow, complete-device, mouse-capability, and platform-profile cells with physical controllers, hot-plug routes, audio-device changes, real window-manager/display checks, captures, declared-hardware frames, and feel/recovery observations | Automation cannot establish device feel, visible behavior, listening, recovery comprehension, or platform integration | Yes - first native alpha and later feel claims |
| 4 | **Repair review findings and publish the first native alpha** - rerun every affected automated and physical gate and create `v0.3.0-alpha.1` only when content, artifacts, provenance, disclosures, and hosted CI agree | This converts reviewed bytes into the first reproducible Windows, macOS, and Linux build that outside players can test | Yes - structured external validation |
...
| 6 | **Continue bounded Python retirement and preview isolation in parallel** - migrate Last Stand fixture generation next, then Phase Shift and the remaining independent fixture-generation paths with tested .NET equivalents while preserving supported-artifact exclusion on every candidate | Native achievement-candidate tooling now establishes exact frozen-vector rendering, freshness, and atomic replacement; Last Stand is the next-smallest targeted corpus with an existing native consumer | No - safe parallel technical work only |
...
**Open release and human gates:** complete the prepared 12-track listening record; move pack `exportEligible` above zero only through reviewed manifests; cut a fresh exact Release candidate; create and complete its schema-2 physical record; retain declared-hardware frame evidence, controller and audio-device passes, platform pixels, and recovery/feel observations; and, for later signed channels, verify signed/notarized candidates plus selected-store delivery. The full source analysis decoded all 95 tracks without source changes or decoder errors but admitted zero as-is. The ignored 12-track `the_bureau` lossless review set passes the provisional `-18 LUFS` plus or minus `2 LU`, `-1 dBTP`, and silence policy with byte-identical repeated output. Listening, source replacement, and pack approval remain explicitly pending. Do not claim 0.8 acceptance while its required human and content gates remain open.
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **1,502** native xUnit contracts pass with 90 percent line and 85 percent branch floors per measured module under Coverlet 10; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; native documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, station-badge, content-inventory, README screenshot, release-material structural, release-rehearsal retained-record, stable-promotion retained-record, exact achievement-candidate fixture rendering/freshness, and Agent Plugin qualification, rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
| --- | --- | --- |
| Release content remains deliberately unavailable | Public inventory is 114 rights-cleared files totaling 342,510,815 bytes, including 95 radio MP3 tracks. Native manifests, strict pack resolution, deterministic packaging, and source-checkout discovery exist, but export eligibility remains zero until pack quality gates pass | The first alpha cannot publish, and the offline content promise cannot be evaluated, until exact content is approved |
...
| Authored feedback is not release-qualified | The typed event matrix, procedural cue fallback, and accessibility alternatives exist. All 95 radio candidates fully decode, but 94 miss the provisional loudness band, 91 exceed the true-peak ceiling, 64 have excessive trailing silence, and 3 have excessive leading silence; no authored player cue or native export radio set is approved | Normalize and trim non-destructive review copies, rerun technical admission, then retain listening and mix evidence on physical devices before approving exact source replacements or manifests |
...
| Release operations found a shared Release-only defect | The first clean manual Release dispatch exposed four localization references to a preview-only assembly; the rule-derived fix and ordinary `ExportRelease` compile gate landed, and exact [run 32421705560](https://github.com/blisspixel/VibeSnake/actions/runs/32421705560) now passes and retains all three Release platforms with provenance | Review the retained artifacts through the physical, content, and protected-delivery gates before publication; keep the ordinary compile regression gate and exact Release rerun on every changed candidate |
...
| --- | --- | --- | --- |
| V030-01 Python reference | Frozen oracle; retirement in progress | Seeded policies, schema 2 reports, per-step invariants, property-generated commands, JSON action traces, immediate replay, 100 movement fixtures with 25,600 steps, 35 targeted core-rule cases, power fixtures (Shield/Phase Shift/Last Stand/remaining), explicit queue acceptance, stable off-path food, normalized random-stream use and respawns, ordered events, explicit `vibesnake-core@4` and randomness-policy declarations, strict source and pack-content gates, and 626 passing Python tests plus 14 subtests measured locally while hosted CI covers 3.11 through 3.14 | Keep the oracle green only until its validators and fixture generators have native replacements; **do not add product features or optional refactors in Python**. Follow V030-13 for complete removal. |
| V030-02 toolchain scaffold | Complete | Godot 4.7.1 and .NET 10.0.303 pins, official editor and .NET template hashes, exact stable SDK resolution, Godot project and application solution, pure rules and test projects, shared fixture readers, export presets, 52-package CI and 4-package runtime hash-locked universal Python graphs with native freshness enforcement, locked NuGet dependencies with transitive vulnerability audit, deterministic path mapping, warnings as errors, formatting, repository-owned local hooks, template bootstrap, and implementation ADR | Reopen only for a dedicated toolchain qualification change |
...
| V030-07 decision close | In progress | **1,502** native contracts pass with 90 percent line and 85 percent branch floors per measured module under Coverlet 10; pure rules throughput evidence JSON with config hash and a 750-step-per-second shared-host regression floor runs in a dedicated non-parallel collection so all-core simulation campaigns cannot overlap its measurement; presentation_frames, required nine-case keyboard/D-pad/stick input-cadence evidence, eight-case viewport-matrix evidence, `shell-presentation-v1`, onboarding, run-end, player-data, audio, SFX, radio, broadcast, `mode-contract-qualification-v2`, `adaptive-fairness-qualification-v1`, deterministic balance-laboratory/observed-baseline/native-AI-league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, fault campaign, native repository checks, and core-only evidence all pass; strict content-pack parsing/isolation and recoverable lifecycle, signing readiness, deterministic package shape, inventory policy, architecture purity, separate RNG streams, preferences schema 7 with schema-1/2/3/4/5/6 migration, input bindings, controller lifecycle, deterministic onboarding, schema-2 fair-category personal bests, versioned top-ten history and Python 0.2 import, replay recording/browse/playback, hosted multi-platform player smoke, and artifact inspection exist. Test failures fail immediately; only invalid coverage output receives one clean retry | Raise Rules and Persistence to the 0.4 acceptance target of 90 percent branch coverage; presentation p50/p95/p99 on declared hardware, physical input and device-change observations, physical audio-device behavior/listening, complexity comparison, and final gate review |
| V030-08 asset inventory | In progress | Strict policy and a generated schema 1 inventory classify and hash 114 public assets totaling 342,510,815 bytes (95 radio MP3s, 9 PNGs, 7 JSON, 3 Markdown); all rights-cleared and structurally valid; 106 blocked for pack export and 8 excluded development references; export-eligible count remains zero until quality and credit gates pass. The full radio analyzer verifies inventory and curation hashes, the complete source set before and after concurrent work, FFmpeg decode, duration, channels, sample rate, EBU R 128 loudness and true peak, clipping risk, and silence. All 95 sources decode with no changes but zero pass as-is. The non-destructive station tool produces 12 technically passing, lossless, hash-bound `the_bureau` review copies; all twelve output hashes repeat exactly, while approval flags remain false | Listen to the first station set on declared devices, approve or reject exact source replacements, regenerate analysis after an approved source change, generate production credits, resolve the duplicate AI personality candidate, select the first export-approved core and radio manifests, and enforce those allowlists in native exports |
...
| V030-13 Python retirement | In progress | The shipped source player and every native artifact already use Godot plus .NET without Python. Native `RepositoryChecks` owns documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, deterministic station-badge, deterministic content-inventory, staged README screenshot capture/freshness, release-material structural qualification, release-rehearsal qualification, stable-promotion qualification, and achievement-candidate fixture rendering/freshness with exact `uv 0.11.33` generation, closed native media validation and generation, bounded integrity checks, canonical atomic JSON generation, structural candidate completion with explicit final-acceptance blockers, and retained external-attestation validation, exact unsigned-to-signed promotion binding, and canonical pending or accepted stable evidence. Python remains frozen test-only scaffolding for remaining validators, fixture generation, pack-tool parity, and the oracle; no product features may land there. [MIGRATION_MAP.md](docs/engineering/MIGRATION_MAP.md#repository-wide-python-retirement) defines the ordered exit gates. | Move Last Stand fixture generation next, then Phase Shift, and the remaining independent fixture-generation paths to .NET; replace the Python CI matrix with equivalent native cross-platform gates; then remove the Python player, tests, packaging metadata, dependency locks, and source-snapshot path in one clean audited sequence. |
...
- **Progress (not closed):** native `RepositoryChecks` owns documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, deterministic station-badge, deterministic content-inventory, staged README screenshot, V090-09 release-material structural qualification, V090-10 release-rehearsal qualification, stable-promotion qualification, and exact achievement-candidate fixture rendering/freshness. Its focused contracts cover malformed inputs, strict JSON and UTF-8, bounded link-free traversal, unsafe patterns and paths, exact pins and hashes, deterministic outputs, atomic replacement, complete PNG integrity, structural candidate media, retained evidence, accepted-material, rehearsal, unsigned-to-signed stable-promotion cross-binding, frozen Python-origin fixture identity, exact bytes, plugin containment, canonical visual generation, inventory classification, media corruption, screenshot source freshness and capture failures, release blockers, and independent command routes. Automated qualification does not perform human approval, staged platform operations, signing, tagging, installation, upload, or publication. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly runs its version and content-release routes after locked restore. Superseded Python commands and duplicate CI gates are removed. Ruff retains full Python syntax ownership while the frozen oracle exists. Last Stand fixture generation is the next bounded native replacement, followed by Phase Shift, the remaining fixtures, and eventual oracle removal.
...
- **Progress (complete automated foundation):** One `ShellTheme` owns the interface font and standard/high-contrast palettes. Retained evidence measures standard primary 15.86:1, standard secondary 8.77:1, and high-contrast primary 21:1 against their canvas backgrounds while enforcing the wider 4.5:1 text and 3:1 essential non-text palette floors. Selection, capture, conflict, bound/unbound, and achievement states use distinct text markers in addition to color. At 150 percent text, the real fallback-font metrics pass width and vertical budgets; bindings use scale-aware rows, the 17-entry achievement catalog is keyboard/controller paged, settings reserve a dedicated description line, all 734 pseudo-localized entries fit, the seven composed Agent Arena overlay rows pass their shared geometry, the worst-case English watch-overlay survival, verification, and outcome rows keep every character, and the six-cell run HUD row keeps every character with a real gutter at every seam and a 14-point floor under composed worst-case English for both mode presentations. Remaining acceptance evidence is retained platform screenshots and human visible-focus/readability review.
...
- Telegraph protection and recovery before consumption so a successful clutch reads as an anticipated player resource with an automatic trigger and a player-controlled recovery window.
...
- **Progress (V090-04 automated gate complete on the retained review build):** Release-mode native CI runs the real exported player 100 consecutive times per Windows x64, macOS Universal, and Linux x64 artifact. Every launch uses a distinct fresh external profile while the install remains read-only, requires a clean exit and explicit launch marker within 30 seconds, and rejects engine warnings, errors, or leaked objects. The packaged smoke mirrors the balanced reference AI for 100,000 compared steps in each of `classic@1` and `vibe@1`, requiring identical decisions, queue outcomes, ordered events, state hashes, and a null first divergence. It creates and advances 100 fresh spectator sessions, verifies initial-state reset, forces collection of prior sessions, and samples Godot node, object, resource, and orphan counts every ten restarts. `candidate-fault-campaign-v1` injects interrupted atomic replacement, corrupt JSON, a disk-full HRESULT, access-denied player data, absent optional resources, invalid content packs, and unavailable audio through production boundaries. Every row must preserve committed data, expose recovery, and leave rules unchanged. Local diagnostics retain privacy-bounded crash and first-divergence reports with exact seed, run, step, hashes, and recent commands. Exact Release run 32421705560 passes and retains the required 300 clean launches, 600,000 compared rules steps, 300 spectator restarts, 21 injected fault rows, both triage probes, and three provenance bundles at revision `e87db6e`. Repeat this gate for any changed candidate; physical and structured-human review remain separate milestones.
...
- **Progress (schema-2 exact-candidate contract and native mouse foundation complete; physical matrix unexecuted):** `vibesnake-manual-product-matrix-v2` defines four exact platform rows for Windows x64, macOS Universal on Apple Silicon, macOS Universal on Intel, and Linux x64. Its atomic observations require 144 platform-flow cells, 432 complete-device flow cells, 16 mouse-capability cells, and 32 platform-profile cells, all bound to one candidate revision and one artifact per platform with an identical Universal artifact hash across both macOS architecture rows. Only passing observations earn coverage; any failed or blocked result remains release-blocking. `prepare_product_review.py` independently recomputes a downloaded Release matrix before projecting exact package and manifest hashes into a candidate record and intentionally incomplete schema-2 templates. The historical `e87db6e` handoff is superseded and contains zero sessions. No current physical candidate exists until approved content lands and a fresh Release run generates exact packages. The native shell supports real mouse actions through the logical input boundary: nine scaled menu targets, left-confirm, right-Back, middle-pause, two-axis wheel navigation, head-relative gameplay direction, and letterbox rejection. `mouse-input-qualification-v1` passes through the real `_Input` route without changing keyboard or controller bindings. The [manual product matrix](docs/release/MANUAL_PRODUCT_MATRIX.md) publishes the execution procedure. Physical execution remains completely open, so V090-07 is not closed.
...
- **Progress (release-material foundation complete; candidate publication pending):** `vibesnake-release-materials-v1` names the ten required README, player, accessibility, privacy, support, recovery, known-issues, notice, credits, and changelog documents; three artifact platforms; four physical input classes; six screenshot roles; two video roles; and eight closed marketing claim IDs. Native `RepositoryChecks` validates that closed foundation with strict duplicate-free JSON, bounded regular files, link-free containment, exact document hashes, and canonical LF `release-materials-handoff-v2` output while retaining `candidateMaterialComplete: false` and `releaseAcceptance: false` without a candidate. A structurally valid candidate sets `candidateMaterialComplete: true` while keeping `releaseAcceptance: false` and the four external acceptance gates pending. A supplied candidate record must match the expected source revision and canonical application version; declare exact artifact-manifest hashes, positive per-platform download and installed bytes, qualified operating-system versions, physical input evidence, offline behavior, save locations, separate core and optional-content bytes, all document hashes, structurally recognized and SHA-256-bound image/video files, and every evidence-linked claim. The structural gate rejects current pending markers, missing media, unsafe paths, malformed containers, unstable retained files, and hash tampering. It never converts artifact-manifest reconciliation, marketing-claim approval, visible image review, or video playback review into approval. Privacy, recovery, known-issues, credits, and third-party-notice foundations are now published, and the accessibility guide no longer points to a disabled issue tracker. Exact candidate screenshots, video, final OS and size disclosures, runtime notices, claim review, visible and playback approval, and a tested public support route remain open, so V090-09 is not closed.
...
- **Progress (native automated foundation complete; accepted materials and staged execution pending):** `vibesnake-release-rehearsal-v1` defines the exact Windows x64, macOS Universal, and Linux x64 candidate, previous-artifact, and manifest records plus eleven required operations per platform: staged download, checksum, signature verification, install, launch, save creation, optional-content install and removal, update, rollback, and application removal. The 33 operation cells require retained `pass` evidence. Candidate artifacts, previous artifacts, manifests, a later `release-materials-acceptance-v1` decision, migration fixtures, every operation file, withdrawal evidence, and authority evidence are covered by exact SHA-256 values. Protected preexisting user-data digests must match before update and after rollback/removal. Withdrawal must make the candidate unavailable, restore the previous artifact, preserve user data, and prepare communication. Publish, halt, replace, and communicate use verified non-personal operational role IDs. Native `release-rehearsal-handoff-v2` binds both machine-readable authorities, the packaging, signing, recovery, and release-material prerequisites, exact revision, record identity, artifact maps, and manifest maps. The foundation retains no rehearsal record and therefore reports false integrity, external-attestation, completion, and acceptance flags. The [release and rollback rehearsal](docs/release/REHEARSAL.md) publishes the procedure. Final signed artifacts, approved optional content, protected roles, and real staged execution remain open, so V090-10 is not closed.
...
- **Progress (stable promotion guard complete; protected execution pending):** `vibesnake-stable-promotion-v1` fixes version and tag `1.0.0`, the three artifact platforms, a separate exact-kind authority for ten mandatory upstream release decisions, seven preserved evidence categories, and six stable compatibility acknowledgements. `stable-promotion-handoff-v2` cannot accept until every upstream decision has its authorized closed schema, ordered gates, retained evidence, and exact revision and version; a protected workflow rebuilds the exact tag; nine review decisions bind one unsigned candidate cohort; protected signing bridges that cohort to each byte-changing public artifact, manifest, provenance bundle, and checksum; the approved optional pack remains separate and hash-bound; each downloaded public file installs and emits the expected deterministic smoke identity; and all retained release records match one complete SHA-256 map. Renamed, copied, manually uploaded, qualification-only, unsigned, unattested, or human-unverified output fails the contract. The native `stable`, `stable-write`, and `stable-record` routes validate or atomically write this evidence. The [stable 1.0 promotion](docs/release/STABLE_PROMOTION.md) publishes the non-mutating verification procedure. No protected promotion record exists, so 1.0 remains queued.
...
| AA-09b | Recorded spectator story | Implemented preview | AA-06, AA-09a | Typed highlights including first-crossing style and lesson beats reconstructed from the named agent tape; deterministic turning-point selection; recorded-first broadcast pacing and montage bound to the exhibition receipt; `vibesnake-agent-exhibition-story-report-v1` loads named lane files and refuses a missing or disagreeing tape; Confirm on `--agent-watch-exhibitions` plays that montage with linger, skip jumps, turning-point seek, a cut on lane switch, rival-lane events from the rival tape, and ordinary pause, seek, speed, restart, and clean-capture controls. Playtest round 6 then made that offer findable from the screen instead of from the integration guide: the selected exhibition row reads `CONFIRM PLAYS THE RECORDED STORY` in place of the bare `WATCH AVAILABLE` state, and the browser carries the same bound-key prompt row the qualification and passport screens already had. Human spectator-appeal evidence remains open |
| AA-10 | Supported symbolic desktop packaging | Active preview | AA-01 through AA-09b | The machine half of current-RID self-contained host packaging is implemented: `package_agent_host.ps1` publishes an unsigned `vibesnake-agent-host-package-v1` with executable, license files, `INSTALL.txt`, closed manifest, lock-derived `host-inventory.json`, unsigned `host-provenance.json`, and complete `SHA256SUMS`; `publication_eligible` stays false; CI builds and validates the package on Windows, macOS, and Linux; the packaged executable opens a live viewer pipe and must not write preview data into the package directory; Godot's headless watch smoke now consumes that pipe. Signing, installers, update, rollback, and supported publication remain |
...
1. Retain the implemented supported-artifact exclusion assertion on all three Release package shapes. Exact allowlists, adversarial filenames, integration bundles, host and viewer assemblies, and compiled Agent Arena command-line markers are automated; exact Release run 32421705560 proves `agentArenaPreviewExcluded: true` on all three retained package manifests. Preserve this proof on every future candidate.
...
6. Playtest round 7 confirmed round 6's three fixes and found that the new exhibitions prompt row advertised the same-seed challenge as `[Unbound]`; the control was bound to R all along and prompt resolution ignored every action outside the eight-action remappable document, so four actions across six screens were misreported. A localization-smoke gate now fails when a prompt names an action nothing binds. Playtest round 6 reached every new room and left one protocol finding and one copy finding, both closed in host `0.18.0`: every rejection decidable from the arguments alone now happens before method binding, leaving only state- and catalog-dependent errors inside the tools, and the exhibition browser names Confirm as the action that plays the recorded story. Playtest AA-08's ranking screen for fairness comprehension. AA-10's current-RID unsigned self-contained host package is implemented with lock-derived inventory, unsigned provenance, a live packaged-host viewer qualification, and a Godot watch smoke against that package; signing and supported publication remain. AA-08's machine half and Godot standings screen are implemented: public decks, an immutable division manifest, eligibility that refuses voluntary `finish_match`, generalization, standings that are not passport ahead/level/behind, Rival Breaker on published terms, and `--agent-watch-qualification`. Human fairness-comprehension evidence remains open. AA-09b's recorded-first story is an implemented preview. AA-07 persistent public identity is implemented as the local passport store. Checked-in AA-05 fixtures are the public qualification-time lesson deck; they are not secret.
...
- Verified by focused automation: exact one-step and zero-step rejection semantics; step-equivalent burst execution, fixed event stops, and all-requirements lesson stops; lesson exact-retry, maximum-32 attempt-witness, independent replay evaluation, factual review, and combined evidence-hash semantics; locked canonical routes plus two non-practice evaluator seeds per lesson; 16 exact action-call, MCP arguments JSON, and structured-response UTF-8 measurements; observation-derived bounded burst routes that use no more calls than paired step routes and fewer for at least six lessons; profile separation; bounded shared mutation idempotency and exhaustion; concurrent serialization; transactional replay-finalization failure; closed observation projection; rival isolation; replay mirror verification; saved replay reload and final-hash playback; capacity-only idle reclamation with an injected monotonic clock; all-tool call-tool argument-name, JSON-type, and closed-value rejection before method binding, including the numeric `gameplaySeed` case and its quoted recovery, catalog-derived vocabularies for lesson, mode, style, rival, action profile, action, and intent, and published bounds for handle, key, state hash, seed, tick, and step-count shape; a stateless official C# SDK subprocess integration transcript; legacy-protocol rejection; protocol-clean stdout; viewer authentication and disconnect isolation; three-platform packaged transcripts launched from the generated `mcp.json`; three-platform self-contained host live-viewer qualification that refuses to write preview data into the package; Godot headless watch smoke against that packaged host; complete package checksum, containment, lock-derived host inventory, and unsigned provenance validation; and knowledge drift. This is not broad client compatibility certification.
- Still required before a supported symbolic Agent Arena release: retained cross-style behavioral-distinction evidence; human legibility of the implemented risk and recovery-resource presentation; human spectator-appeal evidence for the recorded-first story; per-platform signed packages; signed SBOM and artifact inspection; a supported Release export still compiles the watch route out; local league policy beyond the implemented read-time standings; accessibility review; and structured human evidence for pacing, clarity, personality, and rematch desire. The exhibition browser, same-seed handoff, persisted passport store, recorded-first story playback, and local qualification report are implemented in source. Visual-input and other higher-throughput divisions remain separate later options with distinct qualification and rankings. Separately, the implemented supported-1.0 preview-exclusion gate must pass on the exact retained candidate across all three platforms.
...
- Keep one scored rules run on one thread. Use extra cores for independent simulations, background I/O, and hosted operating-system jobs; reduce parallel campaign results in input order so evidence hashes stay stable.
...
| CI passes while the player artifact fails | The first clean manual Release dispatch exposed one shared `ExportRelease` compile defect after ordinary Debug CI passed; the regression gate now compiles `ExportRelease`, and exact three-platform Release run 32421705560 passes | Keep the ordinary compile gate, rerun one cross-bound Release matrix for every changed candidate, and execute selected-channel lifecycle before promotion |
| One platform becomes second-class | The retained `e87db6e` Release matrix passes matching Windows x64, macOS Universal, and Linux x64 gates, but physical and protected delivery evidence remains platform-specific | Keep matching automated acceptance gates, name platform owners for physical review, and never promote with a missing or stale platform artifact |
...
- [EBU R 128](https://tech.ebu.ch/docs/r/r128v4_0.pdf) and [EBU R 128 S2](https://tech.ebu.ch/files/live/sites/tech/files/shared/r/r128s2v1_0.pdf): integrated loudness and true-peak measurement provide the provisional technical screen for offline radio review; they do not replace listening or content approval.
...
1. Close V030-08 and V030-09. Listen to the technically passing, byte-reproducible `the_bureau` review copies on declared devices; approve, revise, or reject each exact replacement; then regenerate the source inventory and technical evidence before approving the minimal core and first radio pack. Record non-zero `exportEligible` allowlists, exact credits, full decode, loudness, true peak, silence, listening, manifest, compressed-size, and installed-size evidence; exports must continue to fail closed until this passes.
2. Freeze the resulting content-bearing revision, run the complete three-platform Release matrix, retain its exact packages, manifests, evidence, and provenance outside ephemeral CI storage, and generate a new schema-2 candidate-bound physical workspace. Historical run `32421705560` remains automated regression evidence only.
3. Execute the physical 0.3 shell review against that fresh candidate. Complete 144 platform-flow cells, 432 keyboard/Xbox-layout/PlayStation-layout device-flow cells, 16 mouse-capability cells, and 32 platform-profile cells while retaining hot-plug, focus, real audio-device, display, platform-capture, recovery-comprehension, and declared-hardware p50/p95/p99 evidence.
...
7. While human sessions, content decisions, or credentials are pending, continue V030-13 after the completed documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, station-badge, content-inventory, screenshot, release-material, release-rehearsal, stable-promotion, and achievement-candidate slices. Move Last Stand fixture generation next, then Phase Shift and one remaining independent fixture generator at a time. Finish or defer each slice before candidate freeze. Remove Python only after the complete replacement matrix passes; never treat new Python features as roadmap progress.
