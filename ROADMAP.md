...
- **Progress (not closed):** `RulesEventCatalog` is the closed wire-name owner; `StarvationWarning` is emitted once at the warning band; `NearMiss` awards are opt-in via `EnableNearMiss`; food precedes near-miss in the same step; `ComboExpired` is opt-in via `EnableComboExpiredEvent` (default off for core_rules parity); `AchievementCandidate` remains reserved without emission until progression wiring lands.
