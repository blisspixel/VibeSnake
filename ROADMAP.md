...
| 6 | **Continue bounded Python retirement and preview isolation in parallel** - replace release rehearsal next, then stable promotion and independent fixture-generation paths with tested .NET equivalents while preserving supported-artifact exclusion on every candidate | Materials now precede rehearsal, and rehearsal precedes stable promotion; the Release matrix remains the earlier candidate producer rather than a downstream material validator | No - safe parallel technical work only |
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **1,449** native xUnit contracts pass with 90 percent line and 85 percent branch floors per measured module under Coverlet 10; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; native documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, station-badge, content-inventory, README screenshot, release-material structural, and Agent Plugin qualification, rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
| --- | --- | --- | --- |
| V030-01 Python reference | Frozen oracle; retirement in progress | Seeded policies, schema 2 reports, per-step invariants, property-generated commands, JSON action traces, immediate replay, 100 movement fixtures with 25,600 steps, 35 targeted core-rule cases, power fixtures (Shield/Phase Shift/Last Stand/remaining), explicit queue acceptance, stable off-path food, normalized random-stream use and respawns, ordered events, explicit `vibesnake-core@4` and randomness-policy declarations, strict source and pack-content gates, and 635 passing Python tests plus 14 subtests measured locally while hosted CI covers 3.11 through 3.14 | Keep the oracle green only until its validators and fixture generators have native replacements; **do not add product features or optional refactors in Python**. Follow V030-13 for complete removal. |
...
| V030-13 Python retirement | In progress | The shipped source player and every native artifact already use Godot plus .NET without Python. Native `RepositoryChecks` owns documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, deterministic station-badge, deterministic content-inventory, staged README screenshot capture/freshness, and release-material structural qualification with 187 focused contracts, exact `uv 0.11.33` generation, closed native media validation and generation, bounded integrity checks, canonical atomic JSON generation, and structural candidate completion with explicit final-acceptance blockers. Python remains frozen test-only scaffolding for remaining validators, fixture generation, pack-tool parity, and the oracle; no product features may land there. [MIGRATION_MAP.md](docs/engineering/MIGRATION_MAP.md#repository-wide-python-retirement) defines the ordered exit gates. | Move release rehearsal next, then stable promotion and independent fixture-generation paths to .NET; replace the Python CI matrix with equivalent native cross-platform gates; then remove the Python player, tests, packaging metadata, dependency locks, and source-snapshot path in one clean audited sequence. |
...
- **Progress (not closed):** native `RepositoryChecks` owns documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, deterministic station-badge, deterministic content-inventory, staged README screenshot, and V090-09 release-material structural qualification. Its 187 focused contracts cover malformed inputs, strict JSON and UTF-8, bounded link-free traversal, unsafe patterns and paths, exact pins and hashes, deterministic outputs, atomic replacement, complete PNG integrity, structural candidate media, retained evidence, plugin containment, canonical visual generation, inventory classification, media corruption, screenshot source freshness and capture failures, release blockers, and independent command routes. Passing structural qualification does not close release acceptance. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly runs its version and content-release routes after locked restore. Superseded Python commands and duplicate CI gates are removed. Ruff retains full Python syntax ownership while the frozen oracle exists. Release rehearsal is the next bounded native replacement, followed by stable promotion, fixture generation, and eventual oracle removal.
