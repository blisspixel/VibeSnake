...
| --- | --- | --- | --- |
| 1 | **Godot shell depth** - remapping UI/glyphs, physical multi-controller evidence, scaling matrix, authored/fallback audio stress, menu/progression polish in `game/` | Players receive the Godot artifact; shell debt is the visible product | Yes - human/feel gates and 0.3 slice close |
| 2 | **Native packaging (V030-10)** - installer/archive shapes, read-only install paths, release manifest schema, signing separation | Continuous export smokes exist; store-shaped install still does not | Yes - install promise and artifact gates |
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **445** native xUnit contracts pass with coverage floors; shared movement, core-rule, power, and achievement-candidate product-path fixtures pass; rules throughput, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 2, input bindings, structured logging, diagnostics, multi-power synergy campaigns, and achievements browse report gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
