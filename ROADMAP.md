...
**Python/Pygame role:** Frozen behavior oracle and temporary validator host only. It is not the 1.0 runtime, not the place for new player features, and not a second product. Reviewed Python-origin fixture bytes remain migration evidence, but their generators now run natively. Keep the reference suite green; do not expand Python gameplay surface unless a parity defect requires an oracle correction.
...
**First 19 safe parallel retirement slices complete:** native `RepositoryChecks` replaces the Python documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, station-badge, content-inventory, README screenshot, release-material, release-rehearsal, stable-promotion, achievement-candidate fixture, Last Stand fixture, Phase Shift fixture, Shield fixture, Remaining Powers fixture, Core Rules fixture, and Movement fixture commands. Its focused malformed-input and deterministic-result contracts run through the combined route on all three hosted operating systems. The four achievement-candidate, five Last Stand, six Phase Shift, eight Shield, nine Remaining Powers, 35 Core Rules, and 100 Movement vectors retain reviewed Python provenance. The seven closed native renderers preserve all 167 vectors and their exact 2,682-byte, 3,596-byte, 3,534-byte, 4,489-byte, 9,548-byte, 57,031-byte, and 999,087-byte canonical LF identities without executing native rules. Their separate C# parity tests remain the live behavior consumers. Six small renderers share a 65,536-byte fixed-fixture lifecycle. Movement uses a separate 1,000,000-byte lifecycle with the same stable-read, safe-path, atomic-replacement, cleanup, and exact self-verification guarantees. Release-material, rehearsal, and stable-promotion qualification remain non-mutating evidence gates with external approval and execution explicit.

**Focused repository-check surface:** 285 contracts after the Movement migration.
...
| 6 | **Continue bounded Python retirement and preview isolation in parallel** - audit the remaining Python CI and package-tool dependency graph, then replace one independently testable authority at a time while preserving supported-artifact exclusion on every candidate | Movement now has a separate tested 1,000,000-byte native lifecycle and exact 999,087-byte identity; Python still owns the frozen oracle, pack-tool parity, and Agent Host package validation, so removal must follow verified native equivalence rather than a bulk deletion | No - safe parallel technical work only |
...
| Automation | Python oracle suite green locally with hosted 3.11 through 3.14 coverage pending for this revision; **1,557** native xUnit contracts pass with zero build warnings and 90 percent line and 85 percent branch floors per measured module under Coverlet 10; RepositoryChecks measures 94.58/86.08 percent line/branch and aggregate native coverage measures 94.29/86.74 percent; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; native documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Knowledge, station-badge, content-inventory, README screenshot, release-material structural, release-rehearsal retained-record, stable-promotion retained-record, exact achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, Core Rules, and Movement fixture rendering/freshness, and Agent Plugin qualification, rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
| --- | --- | --- | --- |
| V030-01 Python reference | Frozen oracle; retirement in progress | Seeded policies, schema 2 reports, per-step invariants, property-generated commands, JSON action traces, immediate replay, 100 movement fixtures with 25,600 steps, 35 targeted core-rule cases, power fixtures (Shield/Phase Shift/Last Stand/remaining), explicit queue acceptance, stable off-path food, normalized random-stream use and respawns, ordered events, explicit `vibesnake-core@4` and randomness-policy declarations, strict source and pack-content gates, and 604 passing Python tests plus 14 subtests at 86.85 percent line coverage measured locally while hosted CI covers 3.11 through 3.14 | Keep the oracle green only until its validators have native replacements; **do not add product features or optional refactors in Python**. Follow V030-13 for complete removal. |
...
| V030-13 Python retirement | In progress | The shipped source player and every native artifact already use Godot plus .NET without Python. Native `RepositoryChecks` owns documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, Agent Knowledge, deterministic station-badge, deterministic content-inventory, staged README screenshot capture/freshness, release-material structural qualification, release-rehearsal qualification, stable-promotion qualification, and exact achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, Core Rules, and Movement fixture rendering/freshness. Six small fixtures share the hardened 65,536-byte lifecycle; Movement uses a separate 1,000,000-byte lifecycle; Agent Knowledge applies the fixed-path lifecycle independently to five closed concepts. The remaining Python CI and package-tool graph is audited and ordered in [MIGRATION_MAP.md](docs/engineering/MIGRATION_MAP.md#repository-wide-python-retirement). Python remains frozen test-only scaffolding for remaining validators, pack-tool parity, and the oracle; no product features may land there. | Move the deterministic local interoperability baseline and digest authority behind an equivalent native cross-platform gate while keeping scheduled upstream network drift isolated; continue through the audited graph; then remove the Python player, tests, packaging metadata, dependency locks, and source-snapshot path in one clean audited sequence. |
...
- **Progress (not closed):** native `RepositoryChecks` owns documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, Agent Knowledge, deterministic station-badge, deterministic content-inventory, staged README screenshot, V090-09 release-material structural qualification, V090-10 release-rehearsal qualification, stable-promotion qualification, and exact achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, Core Rules, and Movement fixture rendering/freshness. Its focused contracts cover malformed inputs, strict JSON and UTF-8, bounded link-free traversal, unsafe patterns and paths, exact pins and hashes, deterministic outputs, atomic replacement, complete PNG integrity, structural candidate media, retained evidence, accepted-material, rehearsal, unsigned-to-signed stable-promotion cross-binding, frozen Python-origin fixture identity, exact bytes, stable bounded reads, separate small- and large-fixture capacity enforcement, destination revalidation, primary-failure-preserving cleanup, plugin containment, canonical visual generation, inventory classification, media corruption, screenshot source freshness and capture failures, knowledge lifecycle dates and closed source-derived catalogs, release blockers, and independent command routes. Automated qualification does not perform human approval, staged platform operations, signing, tagging, installation, upload, or publication. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly runs its version and content-release routes after locked restore. Superseded Python commands and duplicate CI gates are removed. Ruff retains full Python syntax ownership while the frozen oracle exists. The remaining Python CI and package-tool graph is now explicitly audited; the deterministic local interoperability baseline and digest authority is the next bounded native replacement, while its scheduled upstream HTTP probe stays isolated.
...
| Engine migration changes game feel | Pure C# Rules is the product kernel, but the frozen Python oracle still supplies behavior and some validators | Preserve trace and fixture parity, review mismatches, replace bounded Python tooling in .NET, then remove the oracle |
...
7. While human sessions, content decisions, or credentials are pending, continue V030-13 after the completed documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, Agent Knowledge, station-badge, content-inventory, screenshot, release-material, release-rehearsal, stable-promotion, achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, Core Rules, and Movement slices. The remaining Python CI and package-tool graph is audited. Migrate its deterministic local interoperability baseline and digest authority next, keep scheduled network drift separate, and then move one bounded authority at a time in the recorded order. Finish or defer each slice before candidate freeze. Remove Python only after the complete replacement matrix passes; never treat new Python features as roadmap progress.
