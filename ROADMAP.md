...
| AA-10 | Supported symbolic desktop packaging | Active preview | AA-01 through AA-09b | The machine half of current-RID self-contained host packaging is implemented: `package_agent_host.ps1` publishes an unsigned `vibesnake-agent-host-package-v1` with executable, license files, `INSTALL.txt`, closed manifest, and complete `SHA256SUMS`; `publication_eligible` stays false; CI builds and validates the package on Windows, macOS, and Linux; the host smoke-starts over stdio and must not write preview data into the package directory. Viewer artifact qualification, SBOM, provenance, signing, installers, update, rollback, and supported publication remain |
