...
| --- | --- | --- | --- |
| V030-01 Python reference | Frozen oracle; retirement in progress | Seeded policies, schema 2 reports, per-step invariants, property-generated commands, JSON action traces, immediate replay, 100 movement fixtures with 25,600 steps, 35 targeted core-rule cases, power fixtures (Shield/Phase Shift/Last Stand/remaining), explicit queue acceptance, stable off-path food, normalized random-stream use and respawns, ordered events, explicit `vibesnake-core@4` and randomness-policy declarations, strict source and pack-content gates, and 608 passing Python tests plus 14 subtests at 86.85 percent line coverage measured locally while hosted CI covers 3.11 through 3.14 | Keep the oracle green only until its validators have native replacements; **do not add product features or optional refactors in Python**. Follow V030-13 for complete removal. |
...
7. While human sessions, content decisions, or credentials are pending, continue V030-13 after the completed documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, station-badge, content-inventory, screenshot, release-material, release-rehearsal, stable-promotion, achievement-candidate, Last Stand, Phase Shift, Shield, Remaining Powers, Core Rules, and Movement slices. Audit the remaining Python CI and package-tool dependency graph and migrate one bounded authority at a time. Finish or defer each slice before candidate freeze. Remove Python only after the complete replacement matrix passes; never treat new Python features as roadmap progress.
