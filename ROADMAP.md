...
| Identity | Eight radio-station identities, a locally preserved 95-track review library excluded from public source, ten built-in AI personalities, and a custom AI channel |
| Automation | 466 Python tests pass with 3 expected optional-radio skips locally on Python 3.11 through 3.14, with 87.16 percent line coverage measured on Python 3.14; 177 native xUnit contracts pass with 91.73 percent rules line coverage, 90.73 percent persistence line coverage, and 91.55 percent aggregate line coverage; generated native restoration campaigns, live mirror-verified replay recording, bounded atomic replay storage, strict import, resource-bounded canonical replay verification, 100 shared movement traces covering 25,600 steps, 35 targeted core-rule fixtures, 8 targeted Shield fixtures, normalized random-stream and respawn contracts, retained first-divergence evidence, the seeded QA runner, and Godot headless integration smoke pass locally |
| Quality policy | Full-tree Ruff, executable source policy, documentation links, content inventory, shared fixtures, hash-locked Python dependencies, locked audited NuGet restore, compile checks, coverage floors, local pre-commit hooks, and a Python 3.11 through 3.14 CI definition exist |
...
| Source and release content need explicit boundaries | The clean-clone asset tree is 18 rights-cleared files and 95,377 bytes. The ignored `archive/source-assets` collection preserves 423 files and 740,801,845 bytes, including the 95-track, 338,592,122-byte radio review library, retired production tooling, historical production records, and 62 superseded documents, without exposing unresolved media or private workstation history to a public commit | Build size, licensing, update size, and live-content authority require exact manifests rather than recursive packaging |
...
| Display handling is incomplete | Fullscreen switches to the native surface without a viewport, scaling, safe-area, resize, or focus-loss policy | UI and mouse coordinates can become inconsistent away from the checkout resolution |
...
| Authored feedback is not release-qualified | Public source contains no audio binaries; procedural fallbacks cover a small event subset, while unverified SFX candidates remain isolated locally | The game needs a rights-cleared cue set, typed event matrix, deliberate mix, and accessibility alternatives |
...
| Release operations are incomplete | Build presets, checksum-verifying tool bootstrap, artifact smoke and inventory, support policy, and rollback requirements exist; hosted evidence, signing, notarization, promotion controls, and support drills do not | Local Windows evidence alone cannot establish a supportable three-platform release |
...
| V030-03 pure C# slice | In progress | Project-owned coordinates, directions, immutable typed ordered events, run state, PCG32, bounded command queue, bounded grid and cell count, bounded canonical-state and replay imports, wrapping, food, growth, combo interpolation, speed and length scoring, exact starvation order, self-collision precedence, grid completion, the first complete Shield contract, snapshots, explicit terminal restart, strict canonical JSON state schema 2 restoration, explicit `vibesnake-core@4` identity, `fnv1a64-canonical-json-v3` hashes, generated restore-and-continue state machines, a canonical replay schema with logical actions, checkpoints, outcome, fixed compatibility diagnostics, resource-bounded deterministic verification, and SHA-256 payload integrity, plus a live recorder that preserves rejected attempts and checks each Godot step against a private deterministic mirror | Remaining reviewed semantics and migration extraction beyond the current core and Shield scope |
| V030-04 differential parity | In progress | All three shared fixtures declare `vibesnake-core@4` and an exact randomness policy; 100 movement cases provide 25,600 step-level comparisons, 35 core-rule cases span combo, speed, length, score saturation, queue behavior, collision, tail, wrap, starvation, victory, random respawns, and ordered events, and 8 Shield cases span entry collection, pickup and active expiry, collision consumption, expiry precedence, starvation bypass, and collision recovery at the starvation deadline; native generated campaigns repeatedly restore and continue equivalent states; schema 1 first-divergence bundles retain the shortest executed failing prefix, normalized states and events, canonical native state and hash, environment identity, and exact reproduction | Delta reduction beyond the first failing prefix, the other eight powers, and normalization of remaining unported systems |
| V030-05 Godot slice | Prototype working | Real engine launch, menu, logical keyboard and any-controller defaults, buffered movement, focus-loss pause safety, deliberate resume, back and quit flows, simple run rendering, death reason, restart, Music/SFX/UI buses, ten finite cached 16-bit stereo PCM fallback cues, a grid-safe Shield marker, pickup and active countdown text, active-state outline, persistent event captions, typed Shield cue priority, rules and persistence assembly loading, canonical continuation, terminal replay capture, single-flight background save and verification, lossless terminal-save queuing, run-start gating, save-aware quit and window close, latest-replay input, read-only drop import, bounded sanitized compatibility captions, and warning-free clean seeded headless smoke on Windows | Physical controller and hot-plug evidence, remapping and glyphs, scaling matrix, themes, accessibility settings, authored audio, device-failure recovery, and visible feel review |
| V030-06 native artifacts | In progress | Official editor and .NET template archives are pinned; qualification binds the editor executable SHA-256 to the executable inside its verified SHA-512 archive; the Windows x64 debug player exports, launches outside the checkout with an isolated user-data root, records exactly one verified replay without an incomplete temporary file, reports hash `643077d90db75e8c`, and passes a 198-file, 189,615,786-byte schema 2 manifest with no Python runtime, `.env` variant, export lock, checkout path, engine warning, or leaked object; artifact inspection requires and path-scans the Rules, Persistence, and Game payloads; two independent payloads produced manifest SHA-256 `bae7d6369d61c6a57f2fe295f0308c238acc6ccd1e057c20abffc880e8c2ae74` | Retained macOS and Linux runs, menu-to-restart automation, physical controller, audio failure, scaling, screenshots, and broader clean-exit evidence |
| V030-07 decision close | In progress | 177 local native contracts pass at 91.55 percent aggregate line coverage, including 91.73 percent rules and 90.73 percent persistence coverage; formatting, editor smoke, logical input and lifecycle smoke, finite fallback-audio and Shield feedback smoke, replay recording, isolated atomic storage, strict import, bounded background verification, lossless save queuing, save-aware quit smoke, warning and leaked-object rejection, Windows packaged launch, exact artifact measurement, and portability inspection exist | Frame and rules throughput, startup, physical input, audio-device, complexity, macOS and Linux measurements, and final gate review |
| V030-08 asset inventory | In progress | Strict policy and a generated schema 1 inventory classify and hash all 18 clean-clone assets totaling 95,377 bytes; bounded JSON and complete decoded PNG scanline checks pass all files; one duplicate group contains one extra AI personality copy; 7 non-runtime files are excluded and 11 rights-cleared runtime candidates remain blocked for pack review; 423 files and 740,801,845 bytes of rejected, unresolved, generated, analytical, copied-research, superseded-document, private-path, historical production, and working material are preserved only in the ignored `archive/source-assets` collection; unverified audio and retired credentialed production tools are absent from the would-be public commit; pack validation requires exact approved allowlists and matching rights-derived credits | Bind provider-aware provenance for any candidate reconsidered for release, build one audited candidate-admission tool with explicit cost and mutation controls plus a pinned media toolchain, complete full decode, media-quality, lyrics, and content-rating review, resolve the duplicate AI personality candidate, generate production credits, select the first public media set, and enforce approved pack manifests in native exports |
| V030-09 content boundaries | In progress | Schema 1 defines one dependency-free `vibesnake.core`, station-specific radio packs, exact inventory allowlists, canonical encoding, semantic-version and `vibesnake-core@4` ranges, rights-derived credits, file hashes, station track lists, actionable compatibility codes, build-time qualification, and optional failure isolation; compatibility fixtures derive their current rules range from the production identity so a rules bump cannot leave the offline core falsely incompatible | Native content service, first approved core and station manifests, core-only vertical-slice proof, export allowlist integration, removal and tamper UI, and compressed, installed, memory, scan, and startup budgets |
| V030-10 artifact and directory contracts | In progress | Outside-checkout output, platform-specific required Rules, Persistence, and Game payload checks, per-file SHA-256 manifests, prohibited-content rules, deterministic source-path mapping, macOS ZIP inspection, isolated explicit replay roots, spaces and non-ASCII replay paths, bounded file-count and byte budgets, cross-process transaction locking, and no-overwrite atomic replay writes are implemented | Publish exact platform user-data directories; complete installer and archive shapes, read-only install paths, optional-pack removal, signing separation, and release manifest schema |
| V030-11 evidence pipeline | In progress | Workflow defines Python QA, native rules, editor smoke, checksum-verified platform templates, native exports, packaged-player smoke, inspection, and retained bundles on all three systems | Hosted run evidence, broader differential reports, screenshots, content checks, dependency inventories, and provenance retention |
| V030-12 migration and rollback | Not started | High-level migration sequence exists | File-level ownership map, data migration order, rollback procedure, and feature-freeze rules |
...
2. Complete. Shield is the first native power contract. Eight Python-to-C# fixtures prove collection on entry, pickup expiry, active countdown and expiry, one-use collision recovery, expiry-before-collision precedence, starvation bypass, collision recovery at the starvation deadline, normalized state, and ordered events. Native contracts additionally prove deterministic legal spawn, a usable minimum duration, anti-stacking, saturated-board discard, restart, canonical restore, replay verification, and rejection of impossible terminal Shield state.
...
4. Add automated delta reduction beyond the retained first failing prefix and prove each minimized reproducer works from a clean checkout.
