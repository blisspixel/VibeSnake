...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **1,002** native xUnit contracts pass with 90 percent line and 85 percent branch floors per measured module under Coverlet 10; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
| AA-02 | Trustworthy live exhibition | Implemented preview | AA-00, AA-01 | Versioned `vibesnake-agent-viewer-frame-v5`; read-only same-user Godot viewer; exact initial, step, burst, and finish origin; pre-mutation tick and state anchor; actual steps advanced; burst stop reason and event; monotonic sequence and coalescing truth; immutable catalog-bound match identity; exact action acceptance and rejection; exact terminal or failed-closed state; verified-result availability; malformed input clears pending frames; lesson target truth; equal-seed rival; viewer loss cannot affect match state |
| AA-03 | Legible agent character | Active preview | AA-02 | AA-03a implements Passport v2 with closed avatar, accent, and station catalogs, observation v3, viewer frame v5, and agent presentation independent of human cosmetics. Public intent, action result, rival score, and primary Style Contract progress remain visible. Next close honest composite style criteria reconstructed from replay facts, then observed exit, risk, and recovery-resource presentation without inferred intent or route advice. |
...
| AA-09a | Live spectator polish | Implemented preview | AA-02, AA-04 | The current `vibesnake-agent-viewer-frame-v5` preserves AA-09a's closed operation origin and actual advancement bound to a pre-mutation tick and state hash, publishes burst stop and terminal truth, and rejects contradictory or identity-drifting wire facts; the newest-unsent server and latest-frame client expose every sequence gap as coalesced earlier updates; slow control says rules are paused; localized event copy exposes public risk and recovery turns; packaged-host viewer transcript, reduced-motion snap proof, and composed 150-percent pseudo-localized overlay geometry pass without changing rules timing |
...
2. Retain the completed AA-09a automated live-truth gate: frame v4 binds operation kind and actual steps to a pre-mutation tick and state hash, cross-validates immutable identity, action, stop, event, lifecycle, and outcome facts, clears pending content after invalid wire input, proves newest-unsent server coalescing, and retains paused awaiting-agent copy, packaged-host viewer execution, reduced-motion snap, and composed 150-percent-text overlay evidence without letting presentation pace rules.
3. Retain AA-03a identity: Passport v2 accepts only closed avatar, accent, and station IDs; viewer frame v5 rejects unknown or mixed identity; and the agent avatar renders independently of human cosmetics and progression. Complete AA-03b next by qualifying replay-derived composite style criteria under identical rules and seeds, then expose observed risk and resources without route advice.
...
- Complete in source: the current preview decision contracts and initial experience scaffolding; Rules-only session core; local stateless-era stdio MCP 2026-07-28 host; exact one-step play; a separate maximum-16-step event-stopping symbolic burst; shared cross-operation mutation idempotency; capacity-only idle live-handle reclamation without result creation; explicit verified replay save; live read-only same-user pipe viewer with exact action, lesson, and terminal truth; five primary-metric Style Contracts; six selectable and deterministic-route-qualified Signal School practices with primary-metric progress, mutation deltas, target-stopping bursts, and replay-hash-bound outcomes; Passport v2 with closed avatar, accent, and station catalogs; agent presentation independent of human cosmetics; closed presentation-only public intents; equal-seed named rivals; minimal Agent Skill; Agent Plugins 1.0.0 source and package validators; and generated Open Knowledge Format 0.2 concepts with an executable absolute-date freshness gate. Composite replay-derived style truth remains AA-03b work, and representing all eight observable behaviors with eight rejection-aware canonical lessons remains AA-05 work.
...
- Still required before a supported symbolic Agent Arena release: multi-metric style distinction; hash-linked exhibition receipts; per-platform self-contained and signed packages; SBOM and artifact inspection; exported-player and cross-platform viewer qualification; persisted passport and local league policy; public practice and qualification-time seed decks; same-seed human handoff; accessibility review; and structured human evidence for pacing, clarity, personality, and rematch desire. Visual-input and other higher-throughput divisions remain separate later options with distinct qualification and rankings. Separately, the implemented supported-1.0 preview-exclusion gate must pass on the exact retained candidate across all three platforms.
