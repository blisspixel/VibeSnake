...
| V030-03 pure C# slice | In progress | Project-owned coordinates, directions, immutable typed ordered events, run state, PCG32, bounded command queue, bounded grid and cell count, bounded canonical-state and replay imports, wrapping, food, growth, combo interpolation, speed and length scoring, exact starvation order, self-collision precedence, grid completion, complete Shield and Phase Shift contracts (including body-overlap occupancy), snapshots, explicit terminal restart, strict canonical JSON state schema 2 restoration, explicit `vibesnake-core@4` identity, `fnv1a64-canonical-json-v3` hashes, generated restore-and-continue state machines, a canonical replay schema with logical actions, checkpoints, outcome, fixed compatibility diagnostics, resource-bounded deterministic verification, and SHA-256 payload integrity, plus a live recorder that preserves rejected attempts and checks each Godot step against a private deterministic mirror | Remaining reviewed semantics and migration extraction beyond core, Shield, and Phase Shift |
| V030-04 differential parity | In progress | Shared fixtures declare `vibesnake-core@4` and an exact randomness policy; 100 movement cases provide 25,600 step-level comparisons, 35 core-rule cases, 8 Shield cases, and 6 Phase Shift cases (collection, expiry, body overlap, starvation bypass); native generated campaigns repeatedly restore and continue equivalent states; schema 1 first-divergence bundles retain the shortest executed failing prefix, normalized states and events, canonical native state and hash, environment identity, and exact reproduction | Delta reduction beyond the first failing prefix, the remaining seven powers, and normalization of remaining unported systems |
