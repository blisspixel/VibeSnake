...
**Automated review-build prerequisite complete:** [Release run 32421705560](https://github.com/blisspixel/VibeSnake/actions/runs/32421705560) retained the exact `e87db6e` Windows x64, macOS Universal, and Linux x64 packages, manifests, matrix, and provenance. The Release matrix passed 300 clean launches, 600,000 deterministic reliability comparisons, 300 spectator restarts, 21 injected faults, 360 performance samples, and 24 maximum-text-scale display rows. The artifacts remain unsigned review inputs and are not publication eligible.

**Human-review handoffs complete:** `prepare_product_review.py` accepts only a fully recomputed Release matrix, projects its three package hashes into the four physical platform rows, and creates intentionally incomplete session templates. `check_manual_product_matrix.py` now rejects retained sessions without that exact candidate binding. `review_radio_copies.py` rehashes every prepared FLAC and creates a 12-track listening template covering full playback, clipping/distortion, edges, relative level, station fit, and sustained comfort on both headphones and speakers. Both handoffs retain acceptance, publication, source replacement, and export eligibility as false. Physical execution remains 0 of 144 cells and listening remains 0 of 12 decisions.

**First eight safe parallel retirement slices complete:** native `RepositoryChecks` replaces the Python documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, station-badge, and content-inventory commands, has 157 focused malformed-input and deterministic-result contracts, runs the combined route on all three hosted operating systems, and owns tagged-alpha version and content-release validation after locked restore. Source policy covers active authored-file discovery, credentials and signing material, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, and Python placeholders without starting Python. Candidate-freeze policy resolves the 93-file surface, validates inactive or frozen state, and prepares deterministic atomic SHA-256 baselines. Dependency qualification validates both reproducible graphs and directly orchestrates exact `uv 0.11.33` generation with ordered input identity, exact pins and hashes, fatal warnings and timeouts, and atomic replacement. Project-logo qualification pins the approved 1024 by 1024 PNG signature, IHDR dimensions, and SHA-256 identity without an image library. Agent Plugin qualification adds required canonical metadata, bounded strict input and tree traversal, link and launch containment, exact packaged components, and streaming checksum verification. Station-badge qualification owns eight closed station definitions, project glyphs, integer-only drawing, a canonical RGB PNG encoder, bounded link-safe paths, atomic replacement, and exact-byte freshness that rejects valid metadata-only drift. Content-inventory qualification reproduces the 114-file schema-1 snapshot exactly, bounds all inputs and traversal, rejects links and policy ambiguity, streams hashes and media validation, independently verifies PNG zlib and Adler-32 integrity, reports duplicate relationships and release blockers, and writes canonical LF JSON atomically. The superseded inventory command and duplicate Python CI gate are gone; its frozen library remains temporarily for pack-tool parity. Ruff retains complete Python syntax ownership while the frozen oracle exists. Screenshot capture and freshness checking is the next bounded candidate.

| Priority | Next work | Why this order | Blocks the next promotion? |
| --- | --- | --- | --- |
| 1 | **Execute the exact 0.3 product review** - fill the prepared, candidate-bound sessions while running physical controller and hot-plug routes, audio-device changes, real window-manager/display checks, retained platform captures, declared-hardware frame measurement, and visible feel/recovery review against the retained `e87db6e` packages | Artifact identity and record structure are now fail-closed; real devices and human perception are the remaining evidence source | Yes - first native alpha and later feel claims |
| 2 | **Execute and apply the first content decisions (V030-08/09)** - fill the hash-bound 12-track `the_bureau` headphone/speaker record, approve or reject exact source replacements, then produce non-zero `exportEligible` core/radio allowlists with credits, full decode, loudness, true peak, silence, listening, and exact manifests | All 95 source MP3 files decode but fail the complete policy as-is; the first lossless station and its decision record clear every automatable prerequisite without changing approval state | Yes - first native alpha |
| 3 | **Repair review findings and publish the first native alpha** - rerun every affected gate and create `v0.3.0-alpha.1` only when content, artifacts, provenance, disclosures, and hosted CI agree | This converts reviewed bytes into the first reproducible Windows, macOS, and Linux build that outside players can test | Yes - structured external validation |
| 4 | **Run structured human validation and protected delivery work** - execute the qualified playtest cohorts, then complete signing, notarization, selected-channel lifecycle, rollback, and support rehearsals against reviewed candidates | Player evidence should shape polish before 0.8 acceptance; protected publication work must use an accepted artifact rather than a development snapshot | Yes - 0.8, 0.9, and stable promotion |
| 5 | **Retire Python scaffolding in bounded slices (V030-13)** - replace screenshot capture and freshness next, then release and fixture-generation paths with tested .NET equivalents while external reviews or credentials are pending | Content-inventory generation, integrity, exact freshness, and tagged-alpha release readiness are now native; screenshot qualification is the next closed deterministic authority still owned by Python | No - safe parallel technical work only |
| 6 | **Retain supported-artifact preview isolation** - the exact Release matrix proves Agent Arena assemblies, host tools, integration bundles, generated plugin material, and compiled `--agent-watch-*` entry points are excluded; preserve that assertion on every future candidate | Post-1.0 source work can continue without risking accidental inclusion in the supported 1.0 product | Automated foundation and exact review-build evidence complete; retain on future candidates |
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **1,418** native xUnit contracts pass with 90 percent line and 85 percent branch floors per measured module under Coverlet 10; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; native documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, station-badge, content-inventory, and Agent Plugin qualification, rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
| --- | --- | --- | --- |
| V030-01 Python reference | Frozen oracle; retirement in progress | Seeded policies, schema 2 reports, per-step invariants, property-generated commands, JSON action traces, immediate replay, 100 movement fixtures with 25,600 steps, 35 targeted core-rule cases, power fixtures (Shield/Phase Shift/Last Stand/remaining), explicit queue acceptance, stable off-path food, normalized random-stream use and respawns, ordered events, explicit `vibesnake-core@4` and randomness-policy declarations, strict source and pack-content gates, and 637 passing Python tests plus 14 subtests measured locally while hosted CI covers 3.11 through 3.14 | Keep the oracle green only until its validators and fixture generators have native replacements; **do not add product features or optional refactors in Python**. Follow V030-13 for complete removal. |
...
| V030-13 Python retirement | In progress | The shipped source player and every native artifact already use Godot plus .NET without Python. Native `RepositoryChecks` owns documentation, product-version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, deterministic station-badge, and deterministic content-inventory qualification with 157 focused contracts, exact `uv 0.11.33` generation, a closed native RGB PNG encoder, bounded media integrity, canonical atomic JSON generation, and release-blocker enforcement. Python remains frozen test-only scaffolding for remaining validators, fixture generation, pack-tool parity, and the oracle; no product features may land there. [MIGRATION_MAP.md](docs/engineering/MIGRATION_MAP.md#repository-wide-python-retirement) defines the ordered exit gates. | Move screenshot capture and freshness, then release and fixture-generation paths to .NET; replace the Python CI matrix with equivalent native cross-platform gates; then remove the Python player, tests, packaging metadata, dependency locks, and source-snapshot path in one clean audited sequence. |
...
- **Progress (not closed):** the native `RepositoryChecks` command now owns documentation discovery and links, unique changelog contract releases, canonical SemVer parsing, PEP 440 mapping, version alignment across `VERSION`, `pyproject.toml`, the native application identity, and the frozen Python fallback, plus active authored-file discovery, credential and signing-material exclusion, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, Python placeholders, candidate-freeze policy, frozen-surface resolution, active-baseline verification, atomic baseline preparation, both Python dependency-lock profiles, exact `uv 0.11.33` generation with ordered input identity and atomic replacement, the approved 1024 by 1024 project-logo PNG signature, IHDR dimensions, and SHA-256 identity, and source and packaged Agent Plugin validation. One hundred thirty-nine focused contracts cover malformed and missing inputs, excluded and generated trees, duplicate declarations and JSON fields, encoded and invalid paths, fenced examples, inline and multiline placeholders, unsafe globs, prerequisite and activation drift, deterministic hashes and diagnostics, resolver versions, warnings, timeouts, tampering, truncated and non-PNG headers, dimension order, plugin metadata, minimal-skill frontmatter, bounded plugin trees, links, stdio containment, exact packaged components, checksums, and independent command routes. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly runs its version route after locked restore. The superseded Python command files, source-policy module, candidate-freeze command, dependency-lock command, logo and plugin validators, and their replaced tests are removed. Ruff retains full Python syntax ownership. Generated visual, content, screenshot, remaining release validators, fixture generation, and the Python runtime still remain.
...
- **Progress (exact candidate handoff and native mouse foundation complete; physical matrix unexecuted):** `vibesnake-manual-product-matrix-v1` defines four exact platform rows for Windows x64, macOS Universal on Apple Silicon, macOS Universal on Intel, and Linux x64. Its 36-flow allowlist expands every roadmap family into 144 required platform-flow cells, with four device classes, eight audio/accessibility profiles, exact candidate/artifact/session/result schemas, retained safe evidence files, and fail/blocked release rejection. `prepare_product_review.py` independently recomputes a downloaded Release matrix before projecting exact package and manifest hashes into a candidate record and four intentionally incomplete templates. `check_manual_product_matrix.py` rejects retained sessions without that candidate or with mismatched revision, version, or artifact identity, and cannot set `releaseAcceptance` until all cells, devices, and profiles pass against one candidate and one artifact per platform, with one identical Universal artifact hash across both macOS architecture rows. The current `e87db6e` handoff is candidate-qualified with zero sessions. The native shell also supports real mouse actions through the logical input boundary: nine scaled menu targets, left-confirm, right-Back, middle-pause, two-axis wheel navigation, head-relative gameplay direction, and letterbox rejection. `mouse-input-qualification-v1` passes through the real `_Input` route without changing keyboard or controller bindings. The [manual product matrix](docs/release/MANUAL_PRODUCT_MATRIX.md) publishes the execution procedure. Physical execution remains completely open, so V090-07 is not closed.
...
1. Execute the physical 0.3 shell review against the retained `e87db6e` artifacts from [Release run 32421705560](https://github.com/blisspixel/VibeSnake/actions/runs/32421705560): keyboard, mouse, Xbox-layout and PlayStation-layout controllers, hot-plug and focus changes, real audio devices, 4:3 and widescreen window/fullscreen paths, retained platform captures, recovery comprehension, and p50/p95/p99 frames on declared hardware.
2. In parallel, close V030-08 and V030-09. Listen to the technically passing, byte-reproducible `the_bureau` review copies on declared devices; approve, revise, or reject each exact replacement; then regenerate the source inventory and technical evidence before approving the minimal core and first radio pack. Record non-zero `exportEligible` allowlists, exact credits, full decode, loudness, true peak, silence, listening, manifest, compressed-size, and installed-size evidence; exports must continue to fail closed until this passes.
3. Fix every release-blocking finding in C# or Godot, rerun the affected automated and physical gates, and review the resulting exact artifacts. Do not expand the frozen Python player.
4. Create `v0.3.0-alpha.1` only after the reviewed revision has green hosted CI, a complete three-platform Release matrix, detached provenance, approved content, and explicit unsigned-preview disclosure. Download the published files and repeat the short install, launch, input, audio, display, and content check.
5. Use the native alpha for the qualified structured-human sessions. Convert observed control, readability, recovery, power, progression, replay, spectator, audio, and accessibility findings into explicit keep, revise, or remove decisions before claiming 0.8 acceptance.
6. While human sessions, content decisions, or credentials are pending, continue V030-13 after the completed documentation, version, source-policy, candidate-freeze, dependency-lock, project-logo, Agent Plugin, station-badge, and content-inventory slices. Move screenshot capture and freshness next, then continue one bounded validator or fixture generator at a time. Remove Python only after the complete replacement matrix passes; never treat new Python features as roadmap progress.
7. Complete protected signing, notarization, selected-channel install/update/rollback, external validation, release materials, and rehearsal against an accepted candidate before 0.9 or stable promotion.
