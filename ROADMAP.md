...
- **Progress (not closed):** the native `RepositoryChecks` command now owns documentation discovery and links, unique changelog contract releases, canonical SemVer parsing, PEP 440 mapping, version alignment across `VERSION`, `pyproject.toml`, the native application identity, and the frozen Python fallback, plus active authored-file discovery, credential and signing-material exclusion, unfinished markers, prohibited Unicode, attribution policy, strict UTF-8, Python placeholders, candidate-freeze policy, frozen-surface resolution, active-baseline verification, and atomic baseline preparation. Sixty-two focused contracts cover malformed and missing inputs, excluded and generated trees, duplicate declarations and JSON fields, encoded and invalid paths, fenced examples, inline and multiline placeholders, unsafe globs, prerequisite and activation drift, deterministic hashes and diagnostics, tampering, and independent command routes. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly runs its version route after locked restore. The superseded Python command files, source-policy module, candidate-freeze command, and their replaced tests are removed. Ruff retains full Python syntax ownership. Content, screenshot, dependency, remaining release validators, fixture generation, and the Python runtime still remain.
