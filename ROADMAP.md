...
| --- | --- | --- | --- |
| 1 | **Repair and retain the exact 0.3 native review build** - land the `ExportRelease` compile regression exposed by the first clean three-platform dispatch, rerun the matrix, then preserve its packages, manifests, provenance, and qualification evidence | Human and content decisions must refer to exact passing player bytes, and the failed run proved that green Debug CI was not sufficient | Yes - first native alpha |
...
| Release operations found a shared Release-only defect | Hosted multi-platform Debug smokes, deterministic package shapes, provenance routes, promotion guards, release handoffs, and rollback/rehearsal contracts exist. The first clean manual Release dispatch built rules and imported Godot on every platform, then all three exports failed because localization qualification retained four references to a preview-only assembly excluded from `ExportRelease` | Land the rule-derived fix and ordinary `ExportRelease` compile regression gate, rerun the exact matrix, then review the retained artifacts before protected signing or publication |
...
| CI passes while the player artifact fails | The first clean manual Release dispatch exposed one shared `ExportRelease` compile defect after ordinary Debug CI passed; no exact passing three-platform Release candidate is retained yet | Compile `ExportRelease` during ordinary native qualification, then retain one cross-bound Release matrix and execute its selected-channel lifecycle before promotion |
| One platform becomes second-class | Three-platform Debug qualification passes and the first Release dispatch reached the same shared compile defect on all three systems, but no passing retained Release matrix exists yet | Matching acceptance gates, native runners, platform owners, and no 1.0 promotion with a missing Release artifact |
...
1. Land the shared `ExportRelease` fix and its ordinary native compile gate, then dispatch the native workflow in Release mode for that clean `main` revision. Retain all three platform players, the cross-bound release matrix, manifests, checksums, dependency inventory, and provenance so every remaining 0.3 review names exact passing bytes.
