...
**First safe parallel retirement slice complete:** native `RepositoryChecks` replaces the Python documentation and product-version commands, has 35 focused malformed-input and deterministic-result contracts, runs on all three hosted operating systems, and owns tagged-alpha version validation after locked restore. The remaining Python validators and fixture generators stay explicit work under V030-13.
...
| Automation | Python oracle suite green on 3.11 through 3.14 in hosted CI; **1,262** native xUnit contracts pass with 90 percent line and 85 percent branch floors per measured module under Coverlet 10; shared movement, core-rule, power, achievement-candidate, and onboarding fixtures pass; native documentation and version qualification, rules throughput, viewport, shell-presentation/contrast, multimodal and visual-hierarchy budgets, bounded audio allocation/mixing/recovery, manifest-driven radio and broadcast behavior, stable Classic/Vibe mode contracts, bounded adaptive fairness and DDA category isolation, deterministic balance laboratory, observed-baseline, native AI league, schema-2 local-playtest-summary, human-handoff, balance-experiment-guard, score-identity, score-browser, power-decision, replay-browser, offline-comparison, progression, content-curation, creator-content, candidate reliability, and seven-class fault/triage gates, strict content-pack parsing/isolation, atomic archive installation, core-only offline and recoverable optional-pack lifecycle, release signing-readiness, deterministic channel-shape packaging, inventory export, architecture purity, personality, multi-stream RNG, preferences schema 7, input bindings, structured logging, diagnostics, multi-power synergy campaigns, onboarding, run-end/personal-best, separated player-data reset/recovery, achievements browse, deterministic replay browser/playback, and post-1.0 agent-play preview gates; Godot headless and packaged-player smoke pass on hosted Windows, macOS, and Linux runners |
...
- **Progress (not closed):** the native `RepositoryChecks` command now owns documentation discovery and links, unique changelog contract releases, canonical SemVer parsing, PEP 440 mapping, and version alignment across `VERSION`, `pyproject.toml`, the native application identity, and the frozen Python fallback. Thirty-five focused contracts cover malformed and missing inputs, duplicate declarations, encoded and invalid paths, fenced examples, deterministic diagnostics, and independent command routes. CI runs the combined check on Windows, macOS, and Linux; tagged-alpha assembly runs its version route after locked restore. The superseded Python command files are removed. Content, policy, screenshot, dependency, remaining release validators, fixture generation, and the Python runtime still remain.
...
6. While human sessions, content decisions, or credentials are pending, continue V030-13 after the completed documentation/version slice by replacing one bounded Python validator or fixture generator at a time with tested .NET tooling. Source policy is the next small authoritative validator candidate because it protects every later migration change. Remove Python only after the complete replacement matrix passes; never treat new Python features as roadmap progress.
