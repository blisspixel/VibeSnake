...
def test_checked_in_interoperability_baseline_is_aligned_and_fresh() -> None:
    ...
    assert baseline["agent_plugins"]["plugin_version"] == "0.7.1"
    assert baseline["okf"]["spec_version"] == "0.2"
    assert baseline["public_contract_history"]["host"][-1]["version"] == "0.7.1"
    assert baseline["public_contract_history"]["plugin"][-1]["version"] == "0.7.1"
