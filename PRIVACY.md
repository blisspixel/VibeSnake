...
An optional public Agent Passport v4 may contain a bounded caller-declared agent ID, policy version, display name, closed avatar ID, accent ID, station ID, and fixed `symbolic-step-v4` capability profile. The three presentation IDs must resolve through public catalogs and remain per-match, local, and independent of human cosmetics and progression. The declared display name is presentation-only and is never written to disk. After an explicit `record_passport` call the host may keep a local public record assembled only from verified exhibition receipts: agent id, policy versions, division ids, exhibition counts, style and lesson tallies, rival ahead/level/behind counts, and milestones that point at receipt hashes. That store lives under the Godot user-data `agent_arena/` directory, outside supported human player data, and never contains prompts, hidden reasoning, credentials, provider responses, account data, or other personal information.
...
Signal School's v2 lesson contracts use only a published lesson ID and canonical public practice configuration. Every lesson publishes exactly two ordered factual requirements. Rules-trace facts come from the replay. Only `first-turn` retains separate attempt evidence: at most 32 first-seen, valid-state opposite-reversal rejection witnesses containing an ordinal, step or burst operation, a SHA-256 idempotency-key hash, tick, state hash, and action. Raw idempotency keys, prompts, intent, provider data, and human profile data are excluded. Exact retries do not duplicate witnesses. A successful outcome binds replay and attempt evidence hashes. Failed-closed practice exposes no outcome and returns only fresh-session `start_lesson` guidance for the same lesson and action profile. Live practice history is not persisted across host processes except when a verified lesson receipt is later recorded into the local passport store as factual tallies. Completion is evidence of the practice facts rather than mastery.

Style Contract progress contains only rules-advanced-step facts reconstructed from the closed public rules state and events. Rate criteria publish bounded integer numerators and denominators. A successful terminal outcome binds the same two criteria to the verified replay hash. Style evidence does not use prompts, hidden reasoning, provider data, viewer activity, passport identity, or human profile data, and it does not infer intent or mastery.
